...
.PHONY: baseline-test baseline-verify script-static-verify platform-test performance-test production-test production-verify context-test context-verify context-bootstrap context-matrix-check context-post-merge-verify mlops-test mlops-verify local-runtime-test local-runtime-up local-runtime-status local-runtime-smoke-erasure local-runtime-smoke-erasure-recovery local-runtime-logs local-runtime-down local-runtime-reset kind-inotify-configure kind-inotify-verify local-cluster-up local-cluster-verify local-cluster-delete local-istio-ambient-install verify-local-istio-ambient local-istio-ambient-delete local-kyverno-install verify-local-kyverno local-traefik-edge-install verify-local-traefik-edge local-traefik-edge-delete local-observability-install verify-local-observability staging-data-install staging-build staging-deploy staging-verify staging-erasure-recovery production-fidelity-up production-fidelity-verify production-fidelity-down
...
mlops-test:
	$(PYTHON) -m unittest discover -s scripts/mlops/tests -p 'test_*.py'

mlops-verify: mlops-test
	$(PYTHON) scripts/mlops/verify_governance.py

baseline-verify: baseline-test platform-test performance-test production-verify context-test context-verify context-matrix-check mlops-verify local-runtime-test
