...
Ops-MCP tests verify policy fail-closed behavior, allowed/denied-root and symlink escape controls, atomic bounded UTF-8 file mutation, process alias/cwd/timeout/output bounds, child-environment credential exclusion, bounded local audit metadata, CWD-independent startup, and explicit UTF-8 stdio framing. Context-MCP exact read-only tool-surface tests remain mandatory in the same baseline.
