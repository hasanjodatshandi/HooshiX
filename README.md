...
Executable Identity repository slices are present under `services/identity-service/`. The registration slice includes registration/resend/confirmation, PostgreSQL/Flyway persistence, Contact reservation/verification, Argon2id credential hashing, fail-closed Compromised Password screening, ADR-0024 semantic quota, encrypted durable Notification handoff, Day-One observability, hardened deployment artifacts, and service-owned CI/security gates. The current source also includes local-password authentication, server-side Session/RefreshFamily authority, rotating digest-only refresh credentials with reuse revocation, logout, ADR-0024 LOGIN quota behavior, and local ADR-0023 RSA-3072/RS256 signing machinery. The only implemented authenticated session mode is `authenticated_onboarding`; Tenant/Membership selection, MFA/OIDC/password-change/recovery/erasure, ordinary tenant-scoped JWT issuance, deployed runtime evidence, and production readiness remain **NOT VERIFIED**.
