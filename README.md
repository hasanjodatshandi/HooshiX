...
An executable Notification vertical slice is also present under `services/notification-service/`. It includes the internal `SubmitNotification` contract, service-owned PostgreSQL/Flyway persistence, durable acceptance/idempotency primitives, versioned templates, local encrypted delivery material handling, Day-One observability, hardened deployment artifacts, and service-owned CI/security gates. Production Email/SMS.ir provider integration, deployed runtime evidence, and production readiness remain **NOT VERIFIED**.

Executable Identity repository slices are present under `services/identity-service/`. In addition to registration and local-password Session/RefreshFamily/JWT machinery, the current source implements MFA, Google OIDC/ExternalIdentity, password/Profile/Contact lifecycle, complete Tenant/TenantMembership/Invitation coordination, tenant-scoped audience-token issuance, and ADR-0028 data-subject erasure. The erasure slice includes Identity coordination/legal hold, versioned Kafka Protobuf command/receipt events, atomic participant Inbox/Outbox effects in Identity/Authorization/Notification/Web BFF, frontend self-service, finite retry/alerts, restore reconciliation guidance, and a passing four-participant developer-runtime smoke. Real provider execution, production Kafka/erasure deployment, production key/threshold/time-source evidence, load/recovery, and production readiness remain **NOT VERIFIED**.
