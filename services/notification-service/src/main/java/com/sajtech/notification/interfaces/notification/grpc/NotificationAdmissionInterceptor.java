...
public final class NotificationAdmissionInterceptor implements ServerInterceptor {
  private static final Metadata.Key<String> ERROR_CODE =
      Metadata.Key.of("x-hooshix-error-code", Metadata.ASCII_STRING_MARSHALLER);
  private static final String UNAVAILABLE_CODE = "NOTIFICATION_UNAVAILABLE";

  private final Semaphore permits;
  private final AtomicInteger inFlight = new AtomicInteger();
  private final Counter rejected;

  public NotificationAdmissionInterceptor(int maximumConcurrentCalls, MeterRegistry meters) {
    if (maximumConcurrentCalls <= 0) {
      throw new IllegalArgumentException(
          "Maximum global Notification concurrency must be positive");
    }
    permits = new Semaphore(maximumConcurrentCalls, true);
    Gauge.builder("notification.submit.admission.in_flight", inFlight, AtomicInteger::get)
        .register(meters);
    rejected = Counter.builder("notification.submit.admission.rejected").register(meters);
  }
