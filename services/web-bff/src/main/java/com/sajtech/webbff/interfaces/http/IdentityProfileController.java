...
public final class IdentityProfileController {
  ...
  public IdentityProfileController(IdentityGateway identity) { this.identity = identity; }
  ...
  public List<Contact> contacts(HttpServletRequest request) {
    BrowserSession s = HttpSupport.authenticated(request);
    return identity.contacts(s.refreshCredential()).stream().map(c -> new Contact(c.id(), c.type(), c.value(), c.verified(), c.primary())).toList();
  }
  ...
  public Verified verify(@PathVariable UUID id, @RequestBody Verify body, HttpServletRequest request) {
  ...
  public record Add(@NotBlank @Pattern(regexp="EMAIL|PHONE") String type,@NotBlank @Size(max=254) String value) {}
