...
public final class PasswordController {
  ...
  public PasswordController(IdentityGateway identity) { this.identity = identity; }

  @PostMapping("/change")
  public Accepted change(@Valid @RequestBody Change body) {
    return new Accepted(identity.changePassword(body.refreshCredential(), body.currentPassword(), body.newPassword()));
  }
  ...
  public Accepted confirm(@Valid @RequestBody RecoveryConfirm body) {
    return new Accepted(identity.confirmPasswordRecovery(body.contact(), body.code(), body.newPassword()));
  }

  public record Accepted(boolean accepted) {}
  public record Change(@NotBlank String refreshCredential, @NotBlank String currentPassword, @NotBlank String newPassword) {}
  public record RecoveryRequest(@NotBlank String contact) {}
  public record RecoveryConfirm(@NotBlank String contact, @NotBlank String code, @NotBlank String newPassword) {}
}
