CREATE TABLE web_bff_erasure_inbox (
    ...
    CONSTRAINT web_bff_erasure_inbox_completion CHECK (
        (state IN ('PENDING','PROCESSING','EXHAUSTED') AND completed_at IS NULL)
        OR (state='COMPLETED' AND completed_at IS NOT NULL)
    ),
    CONSTRAINT web_bff_erasure_inbox_retention CHECK (
        retain_until >= received_at + INTERVAL '35 days'
    )
);

CREATE INDEX web_bff_erasure_inbox_due_idx
ON web_bff_erasure_inbox(next_attempt_at,event_id)
WHERE state IN ('PENDING','PROCESSING');

CREATE UNIQUE INDEX web_bff_erasure_inbox_request_idx
ON web_bff_erasure_inbox(erasure_request_id);

CREATE TABLE web_bff_erasure_receipt_outbox (
    event_id UUID PRIMARY KEY,
    erasure_request_id UUID NOT NULL,
    participant_policy_version VARCHAR(16) NOT NULL CHECK (participant_policy_version='1'),
    ...
    CONSTRAINT web_bff_erasure_receipt_retention CHECK (
        retain_until >= occurred_at + INTERVAL '35 days'
    )
);

CREATE INDEX web_bff_erasure_receipt_due_idx
ON web_bff_erasure_receipt_outbox(next_attempt_at,event_id)
WHERE state IN ('PENDING','DISPATCHING');

CREATE TABLE web_bff_erasure_evidence (
...
REVOKE UPDATE, DELETE ON web_bff_erasure_evidence FROM PUBLIC;
