...
public final class HmacContactVerificationSecret implements ChallengeSecretPort {
  private static final byte[] DOMAIN =
      "hooshix:identity:contact-verification:v1\0".getBytes(StandardCharsets.US_ASCII);
  private static final Pattern CODE = Pattern.compile("^[0-9]{8}$");
  private final FileBackedKeyRing keys;
  private final SecureRandom random = new SecureRandom();

  public HmacContactVerificationSecret(FileBackedKeyRing keys) {
    this.keys = keys;
  }
  ...
  public GeneratedChallenge generate(UUID challengeId) {
    ...
    return new GeneratedChallenge(code, mac(key.key(), challengeId, code), key.keyId());
  }
  ...
  public boolean matches(UUID challengeId, String code, byte[] verifier, String keyId) {
    ...
    try {
      return MessageDigest.isEqual(computed, verifier);
    } finally {
  ...
  private static byte[] mac(SecretKey key, UUID challengeId, String code) {
    try {
      ...
      throw new IllegalStateException("Contact verification HMAC is unavailable", exception);
    }
  }
}
