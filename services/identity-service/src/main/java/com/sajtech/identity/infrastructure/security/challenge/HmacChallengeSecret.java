...
public final class HmacChallengeSecret implements ChallengeSecretPort {
  private static final byte[] DOMAIN =
      "hooshix:identity:registration-challenge:v1\0".getBytes(StandardCharsets.US_ASCII);
  private static final Pattern CODE = Pattern.compile("^[0-9]{8}$");
  private final FileBackedKeyRing keys;
  private final SecureRandom random;

  public HmacChallengeSecret(FileBackedKeyRing keys) {
    this.keys = keys;
  ...
  private static byte[] mac(SecretKey key, UUID challengeId, String code) {
    try {
      ...
      throw new IllegalStateException("Identity challenge HMAC is unavailable", exception);
    }
  }
}
