...
public final class IdentityPasswordGrpcService
  ...
  public IdentityPasswordGrpcService(
      ChangePassword c, RequestPasswordRecovery r, ConfirmPasswordRecovery f) {
    this.changePassword = c;
    this.requestRecovery = r;
    this.confirmRecovery = f;
  }

  public void changePassword(ChangePasswordRequest r, StreamObserver<ChangePasswordResponse> o) {
    changePassword.change(
        new ChangePasswordCommand(
            r.getRefreshCredential(), r.getCurrentPassword(), r.getNewPassword()));
    o.onNext(ChangePasswordResponse.newBuilder().setChanged(true).build());
    o.onCompleted();
  }

  public void requestPasswordRecovery(
      RequestPasswordRecoveryRequest r, StreamObserver<RequestPasswordRecoveryResponse> o) {
    requestRecovery.request(new RequestPasswordRecoveryCommand(r.getPrimaryContact()));
    o.onNext(RequestPasswordRecoveryResponse.newBuilder().setAccepted(true).build());
    o.onCompleted();
  }

  public void confirmPasswordRecovery(
      ConfirmPasswordRecoveryRequest r, StreamObserver<ConfirmPasswordRecoveryResponse> o) {
    confirmRecovery.confirm(
        new ConfirmPasswordRecoveryCommand(r.getPrimaryContact(), r.getCode(), r.getNewPassword()));
    o.onNext(ConfirmPasswordRecoveryResponse.newBuilder().setChanged(true).build());
    o.onCompleted();
  }
}
