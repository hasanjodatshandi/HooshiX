...
public final class JooqProfileContactStore implements ProfileContactStore {
  ...
  public List<ContactRecord> findContacts(UUID userId) {
    return dsl.fetch(
            "SELECT contact_id,contact_type,delivery_value,verified_at,primary_active FROM identity_contact WHERE user_id=? AND removed_at IS NULL",
            userId)
        .map(
            r ->
                new ContactRecord(
  ...
  public UUID addContact(UUID userId, String type, String value) {
    ...
    dsl.execute(
        "INSERT INTO identity_contact(contact_id,user_id,contact_type,canonical_value,delivery_value,created_at,updated_at) VALUES (?,?,?,?,?,?,?)",
        id,
        userId,
        type,
        value.toLowerCase(),
  ...
  public boolean verifyContact(UUID userId, UUID contactId, String code) {
    return dsl.execute(
            "UPDATE identity_contact SET verified_at=?,updated_at=? WHERE contact_id=? AND user_id=? AND removed_at IS NULL",
            Instant.now(),
            Instant.now(),
            contactId,
            userId)
  ...
  @Override
  public boolean setPrimary(UUID userId, UUID contactId) {
    dsl.execute(
        "UPDATE identity_contact SET primary_active=FALSE,updated_at=? WHERE user_id=?",
        Instant.now(),
        userId);
    return dsl.execute(
            "UPDATE identity_contact SET primary_active=TRUE,updated_at=? WHERE contact_id=? AND user_id=? AND removed_at IS NULL",
            Instant.now(),
            contactId,
            userId)
        == 1;
  ...
  public boolean remove(UUID userId, UUID contactId) {
    return dsl.execute(
            "UPDATE identity_contact SET removed_at=?,updated_at=? WHERE contact_id=? AND user_id=? AND removed_at IS NULL",
            Instant.now(),
            Instant.now(),
            contactId,
            userId)
