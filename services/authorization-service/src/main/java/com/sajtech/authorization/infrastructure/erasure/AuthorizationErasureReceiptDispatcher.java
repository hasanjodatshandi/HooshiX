...
public final class AuthorizationErasureReceiptDispatcher {
  ...
  public AuthorizationErasureReceiptDispatcher(
      DSLContext dsl,
    ...
    this.clock = clock;
    this.topic = topic;
    published = meters.counter("authorization.erasure.receipt_outbox.published");
    failures = meters.counter("authorization.erasure.receipt_outbox.failures");
  }

  @Scheduled(fixedDelayString = "${authorization.erasure-receipt-dispatch-delay:PT1S}")
  public void dispatch() {
    ...
    try {
      ...
      transactions.executeWithoutResult(
          ignored ->
              dsl.execute(
                  "UPDATE authorization_erasure_receipt_outbox SET state='PUBLISHED',published_at=CAST(? AS TIMESTAMP WITH TIME ZONE),lease_until=NULL,last_error_class=NULL,updated_at=CAST(? AS TIMESTAMP WITH TIME ZONE) WHERE event_id=? AND state='DISPATCHING'",
                  ts(done),
                  ts(done),
                  item.eventId()));
      published.increment();
  ...
  private Optional<Item> claim(Instant now) {
    return dsl.fetchOptional(
            "SELECT event_id,erasure_request_id,participant_policy_version,action_categories,attempt_count,occurred_at FROM authorization_erasure_receipt_outbox WHERE state IN ('PENDING','DISPATCHING') AND next_attempt_at<=CAST(? AS TIMESTAMP WITH TIME ZONE) AND (lease_until IS NULL OR lease_until<=CAST(? AS TIMESTAMP WITH TIME ZONE)) ORDER BY next_attempt_at,event_id LIMIT 1 FOR UPDATE SKIP LOCKED",
            ts(now),
            ts(now))
        .map(
            row -> {
              ...
              dsl.execute(
                  "UPDATE authorization_erasure_receipt_outbox SET state='DISPATCHING',lease_until=CAST(? AS TIMESTAMP WITH TIME ZONE),updated_at=CAST(? AS TIMESTAMP WITH TIME ZONE) WHERE event_id=?",
                  ts(now.plus(LEASE)),
                  ts(now),
                  id);
              return new Item(
    ...
    transactions.executeWithoutResult(
        ignored -> {
          if (attempt >= MAX_ATTEMPTS) {
            dsl.execute(
                "UPDATE authorization_erasure_receipt_outbox SET state='EXHAUSTED',attempt_count=?,lease_until=NULL,last_error_class=?,updated_at=CAST(? AS TIMESTAMP WITH TIME ZONE) WHERE event_id=? AND state='DISPATCHING'",
                attempt,
                error,
                ts(now),
                item.eventId());
            ...
            dsl.execute(
                "UPDATE authorization_erasure_receipt_outbox SET state='PENDING',attempt_count=?,next_attempt_at=CAST(? AS TIMESTAMP WITH TIME ZONE),lease_until=NULL,last_error_class=?,updated_at=CAST(? AS TIMESTAMP WITH TIME ZONE) WHERE event_id=? AND state='DISPATCHING'",
                attempt,
