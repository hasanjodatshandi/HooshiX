...
HooshiX already requires blocking source analysis, dependency integrity, final-artifact SBOM/vulnerability correlation, signed provenance, and fail-closed production admission. The current documents select Semgrep, Syft, Grype, Cosign, and Kyverno for parts of that chain, but they do not define one exact responsibility map and they do not select a dedicated Git secret scanner.

Overlapping scanners can create duplicated findings without closing a distinct failure class. Missing responsibility boundaries can also cause false assumptions, such as treating Gradle dependency verification as CVE scanning or treating the use of Semgrep CLI as proof that separate Semgrep Secrets or Supply Chain products are enabled.
...
| final-image/SBOM vulnerability correlation | Grype | ADR-0035/0038 severity, freshness, exception, and deployed-digest policy |
...
### 4. Final-artifact SBOM and vulnerability correlation
...
Grype consumes the final image/SBOM for vulnerability correlation so operating-system packages, transitive application dependencies, the JDK/runtime, and packaged native components remain visible. ADR-0035 and ADR-0038 remain authoritative for severity gates, feed freshness, continuous rescanning, VEX/exception behavior, ownership, and response targets.

Gradle dependency verification and locks remain mandatory but are not a replacement for Grype/SBOM vulnerability analysis.

### 5. Signing, provenance, and admission
...
### 6. Tools not selected in the current baseline

Trivy is **not** selected as an additional default scanner in the current baseline. Its image/dependency/SBOM/IaC capabilities substantially overlap the selected Syft + Grype + existing Kubernetes/Helm/Kyverno validation chain. It may be reconsidered only when a measured coverage gap identifies a distinct failure class that the current controls do not adequately detect.

OWASP Dependency-Check is **not** selected as an additional default Java SCA gate in the current baseline. Final-artifact/SBOM vulnerability correlation is owned by Syft + Grype, while Gradle verification/locks own dependency integrity. Dependency-Check may be reconsidered if evidence shows a JVM/ecosystem vulnerability-identification gap that is not adequately covered by the selected final-artifact pipeline.
...
### 8. Tool integrity and privileges
...
Scanner databases/feeds and tool failures follow the freshness/fail-closed promotion semantics already defined by ADR-0035 and ADR-0038. A scanner outage is not permission to disable the gate or silently reuse stale evidence beyond the approved window.
...
- Each service/deployed-artifact owner owns remediation of first-party findings and all direct/transitive findings present in its final artifact.
...
- Gradle verification failure and vulnerability-scan failure remain separate failure classes;
...
- no production promotion can bypass a required secret/SAST/SBOM/vulnerability/signature/provenance/admission gate;
...
Documentation alone is not implementation evidence. Until the corresponding CI/release controls exist and execute successfully, their status remains `NOT PRESENT` or `NOT VERIFIED` as appropriate.

## Relationship to current decisions

This ADR complements ADR-0017, ADR-0035, ADR-0038, and ADR-0039. It does not supersede their artifact-signing, vulnerability-response, exception, or executable-quality semantics. It adds the missing dedicated secret-scanning decision and makes the selected tool responsibilities explicit.
...
Rollback or tool replacement MUST preserve equivalent or stronger secret detection, source SAST, final-artifact inventory/vulnerability correlation, signature/provenance/SBOM evidence, and fail-closed production admission.

Rollback MUST NOT return to no committed-secret scanning, source-only vulnerability visibility, unsigned production artifacts, unverified provenance/SBOM, or admission that accepts missing required evidence. A tool may be replaced only through a reviewed change that preserves the owned failure classes and evidence contracts.