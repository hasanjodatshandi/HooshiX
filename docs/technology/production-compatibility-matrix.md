...
| Kubernetes topology | 3 stacked control-plane/etcd + >=3 workers | ADR-0022; redundant API endpoint and one-node-loss evidence |
...
| PostgreSQL | 18.4 | CloudNativePG 1.30-supported major/patch; ADR-0027 dedicated database/cluster/roles; ADR-0034 fleet model; not used for ADR-0040 immutable reference dataset |
...
| Kyverno | 1.18.2 | stable policy/image-validation APIs; admission-policy authoring RBAC plus bounded CEL HTTP-context egress/SSRF controls from ADR-0017 must remain valid on Kubernetes 1.35 |
...
- Compromised Password SQLite Java/native/dataset-format/read-only compatibility and representative indexed lookup behavior when ADR-0040 is affected;
