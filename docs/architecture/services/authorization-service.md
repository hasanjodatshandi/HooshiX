...
The current repository implementation contains the executable Authorization slice under `services/authorization-service/`, including contracts, four PostgreSQL/Flyway migrations with forced RLS, online checks/management commands, bounded `CheckPermission` admission, owner-removal safety, Redis quota, audit/idempotency, immutable key identifiers, and hardened deployment artifacts. V4 adds the ADR-0028 participant: validated Kafka input, atomic Inbox/idempotency, Identity-owned target resolution, removal of subject-linked tenant/platform authority while preserving tenant-owned policy, non-PII evidence/receipt Outbox, finite retry/exhaustion, Kafka network policy, metrics, and alerts. Current canonical WSL Java 25 full Gradle verification, PostgreSQL/Redis integration, Buf compatibility, exact Helm render policy, Prometheus rule validation, and the real local four-participant Kafka smoke pass. The protected Authorization service suite passed on `main@68cf66c` in repository baseline run `33105936814`; prior production-fidelity kind/staging evidence covers V3 and therefore predates this revision. Kind/staging V4 redeployment, production Kafka/runtime, and production readiness remain `NOT VERIFIED`.
