...
The canonical browser-facing contract is BFF-owned OpenAPI 3.1, version 1.4.0, under `services/web-bff/contracts/openapi.yaml`. It covers all 48 implemented public controller method/path mappings. A compatible contract change follows SemVer, contains schema validation and a consumer example for every operation, preserves controller/OpenAPI parity, and regenerates frontend transport types; the generated-type drift gate rejects stale client output.
