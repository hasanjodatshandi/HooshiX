...
The canonical browser-facing contract is BFF-owned OpenAPI 3.1, version 1.5.0, under `services/web-bff/contracts/openapi.yaml`. It covers all 57 implemented public controller method/path mappings. A compatible contract change follows SemVer, contains schema validation and a consumer example for every operation, preserves controller/OpenAPI parity, and regenerates frontend transport types; the generated-type drift gate rejects stale client output.
...
The current repository contains the executable Web BFF slice under `services/web-bff/`, including server-side Redis sessions, same-origin/CSRF/Fetch-Metadata enforcement and reload recovery, local password+MFA authentication/logout, Google Authorization Code with PKCE S256/state/nonce, bounded encrypted pre-auth state, exact redirect/return-target validation, issuer/audience/time/nonce/claim validation, provider-token custody, OIDC semantic quota, ExternalIdentity status/link/unlink, TOTP lifecycle routes, complete Tenant lifecycle/selection and Invitation-management routes, profile/contact routes, password change/recovery routes, Authorization-management routes, exact audience brokerage, and the canonical OpenAPI 3.1 version-1.5.0 browser facade. Version 1.5.0 establishes 57-route controller coverage, consumer examples, schema validation, controller/OpenAPI parity, and generated frontend transport-type drift enforcement. Tenant deletion of the selected tenant rotates the browser into onboarding state with a fresh CSRF binding rather than preserving stale tenant authority. Provider access/ID tokens and client secret never reach browser or Identity; only typed bounded evidence crosses the one-attempt 1500ms Identity edge. Login/link/MFA completion and external-identity mutation rotate browser/server security state as required. OIDC start/callback use exact-IP hard identity, separate `/24`/`/64` aggregate pressure, common-clock/Redis-time/host-sync and noeviction/headroom/cardinality fail-closed controls. Application ports retain ArchUnit enforcement. Production BFF runtime and real Google-provider execution remain `NOT VERIFIED`.
