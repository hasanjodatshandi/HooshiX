...
The canonical browser-facing contract is BFF-owned OpenAPI 3.1, version 2.1.0, under `services/web-bff/contracts/openapi.yaml`. It covers all 67 implemented public controller method/path mappings, including authenticated self-erasure and the private Conversation journey. A compatible contract change follows SemVer, contains schema validation and a consumer example for every operation, preserves controller/OpenAPI parity, and regenerates frontend transport types; the generated-type drift gate rejects stale client output.
