...
The first executable implementation is repository-complete only when source, contracts, migrations, tests, build/dependency locks, container/deployment/security policy, **observability**, and CI gates for the implemented slice exist. The current repository implementation includes registration, local-password authentication/Session/RefreshFamily/JWT-signing, Tenant/Membership/Invitation lifecycle, selection, Authorization coordination, tenant-authenticated session state, and audience-token behavior. Prior protected evidence remains commit-specific: registration/authentication evidence passed through `main@a3766bd`, and the expanded five-service protected PR baseline passed the then-current Identity integration/security/deployment-package gates on implementation head `7de8b17` in run `32261626399`. Current local revision evidence additionally covers transaction-local post-authentication tenant selection, non-owner forced-RLS missing/malformed/pool-reuse negatives, five Flyway migrations including V5 tenant/session Membership identity constraints, immutable symmetric/RSA key identifiers across refresh, bounded global zero-queue gRPC admission, Tenant lifecycle operation telemetry, indexed Authorization-outbox age telemetry, ADR-0012 pending-age/definitive-failure alert rules, strict Gradle/architecture/SpotBugs gates, Buf compatibility, Semgrep, OSV, Gitleaks, Helm render hardening, Prometheus/dashboard validation, and digest-pinned runtime-image construction. The current V5 migration revision is redeployed in the local production-fidelity kind/staging lane as part of the exact-digest five-service stack, including Identity-to-Authorization through Ambient workload identity, edge/WAF verification, telemetry privacy canaries, and telemetry-backend outage verification. Production deployed runtime evidence remains `NOT VERIFIED`. MFA/OIDC/password-change/recovery/erasure, load/recovery, production signing-key rotation, and production readiness also remain `NOT VERIFIED` until their owning gates execute.
