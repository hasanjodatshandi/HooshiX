...
The first executable implementation is repository-complete only when source, contracts, migrations, tests, build/dependency locks, deployment/security policy, observability, and CI gates exist. The current repository includes registration, local-password and Google OIDC authentication, Session/RefreshFamily/JWT signing, ExternalIdentity, password/Profile/Contact, TOTP MFA/recovery, complete Tenant/Membership/Invitation lifecycle, Authorization coordination, tenant session selection, audience-token behavior, and Notification result handling. V13 adds ADR-0028 coordination and the Identity-local participant: recent-auth/MFA and Membership checks, atomic `DELETING` authentication shutdown/invitation revocation, legal-hold authorization and ledger, server-owned participant snapshots, Transactional Outbox, validated versioned Kafka events, atomic Inbox/idempotency, service-owned erasure, non-PII receipts, receipt-gated completion, 35-day evidence, finite retry/exhaustion, metrics/alerts, Helm policy, and restore/replay guidance. Current local evidence covers thirteen Flyway migrations, strict full Gradle checks, contract/Buf compatibility, PostgreSQL/Redis integrations, hardened Helm/Prometheus, five simultaneously ready services, and a real four-participant Kafka completion smoke. The protected Identity service suite passed on `main@68cf66c` in repository baseline run `33105936814`; kind/staging redeployment of the erasure revision, production Kafka/erasure, real Google-provider execution, load/recovery, production signing-key rotation, and production readiness remain `NOT VERIFIED`.
