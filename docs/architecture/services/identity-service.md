...
The first executable implementation is repository-complete only when source, contracts, migrations, tests, build/dependency locks, container/deployment/security policy, **observability**, and CI gates for the implemented slice exist. The current repository implementation includes registration, local-password authentication/Session/RefreshFamily/JWT-signing, password change/recovery/reset, Tenant/Membership/Invitation lifecycle, selection, Authorization coordination, tenant-authenticated session state, audience-token behavior, persistence of the Notification service notification identifier after durable handoff, and an idempotent terminal Notification result callback on a dedicated gated listener. Prior protected evidence remains commit-specific. Current local revision evidence covers transaction-local post-authentication tenant selection, non-owner forced-RLS negatives, eight Flyway migrations including V5 tenant/session Membership identity constraints, V6 Notification result correlation, and V7/V8 password recovery/outbox/session-revocation state; immutable key identifiers, bounded gRPC admission, Tenant/password telemetry, strict Gradle integration/architecture/SpotBugs gates, Buf compatibility, Semgrep, OSV, Gitleaks, Helm render hardening, Prometheus/dashboard validation, and digest-pinned runtime-image construction. Existing local production-fidelity evidence predates this password revision, so deployed password lifecycle remains `NOT VERIFIED`. MFA/OIDC/erasure, load/recovery, production signing-key rotation, and production readiness also remain `NOT VERIFIED` until their owning gates execute.
