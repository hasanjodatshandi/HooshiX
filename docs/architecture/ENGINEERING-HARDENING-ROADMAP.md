...
| 9 | ADR-0054 private Conversation + ModelRun vertical slice | `IN PROGRESS` | Complete the two non-fabricable activation gates: real provider evaluation plus provider-account data-control/owner approvals, then exercise the reviewed canary and rollback path. | The service/contracts/DB/RLS/encryption/worker/provider/cost/lifecycle/telemetry/BFF/UI slice is implemented, including enum-only feedback and erasure, refusal/safety mapping, local foreground transport cancellation, signed content-free evaluation receipts, provider metrics/alerts/dashboard/runbook, fixed conditional `api.openai.com:443` egress, and deterministic `CANARY_1/5/25/APPROVED_100` cohorts. Runtime remains fail-closed because no real evaluation receipt or provider-account approval evidence is present; deployed canary observation and rollback rehearsal are therefore `NOT VERIFIED`. Earlier Stage 9 merge evidence remains recorded in the application roadmap; active PR evidence is commit-specific and must pass before merge. |
