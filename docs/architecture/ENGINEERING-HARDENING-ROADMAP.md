...
| 9 | ADR-0054 private Conversation + ModelRun vertical slice | `IN PROGRESS` | Implement the accepted service/contracts/DB/RLS/encryption/worker/provider/cost/lifecycle/telemetry/BFF/UI slice and its security/privacy/failure/load/browser/Helm evidence, preserving every ADR-0054/0057 exclusion and activation gate. | Neutral contracts, executable service foundation, authority boundary, and private Conversation CRUD/lifecycle state are complete for the current repository slice. CRUD now has validated gRPC transport, exact permission checks, Membership ownership, forced RLS, encrypted titles, bounded opaque pagination, optimistic versioning, durable request replay/conflict semantics, stable non-leaking errors, bounded listener concurrency, and safe telemetry. Message/ModelRun acceptance and cancellation are the next ordered slice; worker/provider/cost, tenant lifecycle/erasure, BFF/UI, release and deployed-runtime evidence remain pending. |
