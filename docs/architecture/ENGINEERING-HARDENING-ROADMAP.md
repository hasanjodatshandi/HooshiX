...
| HR-017 | HIGH | Core product | `IN PROGRESS`: the neutral contract and executable Conversation service foundation now exist with owned build/CI, Flyway schema, forced RLS, AES-256-GCM key handling, observability, and hardened Helm. Authorization/API/state use cases, lifecycle/erasure, worker/provider/cost, BFF/UI, release integration, and runtime evidence remain. | 9 |
...
| 9 | ADR-0054 private Conversation + ModelRun vertical slice | `IN PROGRESS` | Implement the accepted service/contracts/DB/RLS/encryption/worker/provider/cost/lifecycle/telemetry/BFF/UI slice and its security/privacy/failure/load/browser/Helm evidence, preserving every ADR-0054/0057 exclusion and activation gate. | Neutral contracts, executable service foundation, authority boundary, private Conversation CRUD/lifecycle, encrypted USER Message history, exact governed model/prompt/price selection, and idempotent ModelRun acceptance/get/queued-running cancellation are implemented in the active slice. Acceptance atomically reserves tenant and Membership budget; queued cancellation/delete release reservations; running cancellation retains reservation for later reconciliation. Full strict/protected evidence for this active slice remains pending. Worker claim/provider execution/completion and cost reconciliation, tenant lifecycle/ADR-0028 erasure, BFF/UI, release and deployed-runtime evidence remain pending; provider execution remains disabled by current governance. |
