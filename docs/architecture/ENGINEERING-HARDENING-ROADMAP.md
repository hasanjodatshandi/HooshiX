...
| 9 | ADR-0054 private Conversation + ModelRun vertical slice | `IN PROGRESS` | Record exact provider-account data-control and named owner approvals, then exercise the reviewed canary and rollback path. | The service/contracts/DB/RLS/encryption/worker/provider/cost/lifecycle/telemetry/BFF/UI slice is implemented, including enum-only feedback and erasure, refusal/safety/content-filter mapping, local foreground transport cancellation, signed content-free evaluation receipts, provider metrics/alerts/dashboard/runbook, fixed conditional `api.openai.com:443` egress, and deterministic `CANARY_1/5/25/APPROVED_100` cohorts. The signed v2 real-provider receipt passed 12/12, 8/8 critical, zero errors, p95 4.986s, and $0.033958 total at evaluated commit `bc5af2853ed08d5d4f4bbd39f77bda3feac49f2c`. Runtime remains fail-closed because provider-account approval evidence is absent; deployed canary observation and rollback rehearsal are therefore `NOT VERIFIED`. Earlier Stage 9 merge evidence remains recorded in the application roadmap; active PR evidence is commit-specific and must pass before merge. |
