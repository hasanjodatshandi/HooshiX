...
| 9 | ADR-0054 private Conversation + ModelRun vertical slice | `IN PROGRESS` | Implement the accepted service/contracts/DB/RLS/encryption/worker/provider/cost/lifecycle/telemetry/BFF/UI slice and its security/privacy/failure/load/browser/Helm evidence, preserving every ADR-0054/0057 exclusion and activation gate. | Neutral contracts and the executable service foundation are complete. The Conversation authority boundary now validates exact Identity JWT issuer/signature/time/`aud=conversation-service`, fixes the four permission keys, performs one bounded 300ms fail-closed Authorization call without retry/wait/cache, exposes stale-key readiness, and carries least-privilege mesh/network configuration. Conversation/run state use cases are the next ordered slice; overall Stage 9 completion remains pending. |
