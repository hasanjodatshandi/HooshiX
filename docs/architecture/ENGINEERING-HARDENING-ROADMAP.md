...
| 9 | ADR-0054 private Conversation + ModelRun vertical slice | `IN PROGRESS` | Implement the accepted service/contracts/DB/RLS/encryption/worker/provider/cost/lifecycle/telemetry/BFF/UI slice and its security/privacy/failure/load/browser/Helm evidence, preserving every ADR-0054/0057 exclusion and activation gate. | Neutral contracts, executable service foundation, authority boundary, private Conversation CRUD/lifecycle, encrypted Message history, exact governed model/prompt/price selection, idempotent ModelRun acceptance/get/cancellation, atomic tenant/Membership reservation, durable metadata-only `SKIP LOCKED` worker leases, fixed one-attempt stateless OpenAI adapter, completion/cancellation/unknown-outcome handling, and integer actual-cost reconciliation are implemented in the active slice. Current governance and cluster egress keep execution disabled. Local strict service, migration/integration, architecture, coverage, SpotBugs, OSV, Semgrep, Gitleaks, Helm and repository-baseline evidence passes for this worker/provider slice. PR #134 protected baseline run `35466974425` and frontend E2E run `35466974204` passed; post-merge baseline run `35467376618` and frontend E2E run `35467376506` passed at exact merge `2f450003fa50f0e83ce3321799d922e2467e4d92`. Tenant lifecycle/ADR-0028 erasure, provider safety/evaluation/telemetry and transport cancellation, BFF/UI, activation egress, release and deployed-runtime evidence remain pending. |
