...
| 8 | MLOps evaluation and safety architecture gate | `IN PROGRESS` | Approve versioned non-PII eval data, model/prompt/price catalog, promotion/rollback/canary thresholds, safety/acceptable-use/feedback/drift policy, and provider data-control requirements. Do not add an MLOps platform without an evidenced need. | Draft PR #127; governance/schema/validator implementation is under verification |
