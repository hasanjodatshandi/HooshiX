...
| HR-015 | HIGH | MLOps governance | `CLOSED FOR STAGE 8`: ADR-0057 and the machine-validated Git assets define a versioned offline synthetic/adversarial suite, quality/safety/cost/latency promotion thresholds, canary policy, and model/prompt rollback contract. Stage 9 subsequently exercised the real evaluation/one-percent canary/rollback path; later promotion and Production remain gated. | 8 |
| HR-016 | HIGH | AI safety/data control | `CLOSED FOR STAGE 8`: Conversation owns safety/acceptable-use decisions; provider-account data-control approval fails closed; enum-only feedback, drift triggers, non-PII evaluation handling, and no automatic production-content training are explicit and validated. Staging approval/execution passed in Stage 9; Production approval remains Stage 10 evidence. | 8 |
| HR-017 | HIGH | Core product | `CLOSED FOR STAGE 9`: the neutral contract and executable Conversation service include Authorization/private CRUD/history, idempotent ModelRun acceptance/cancellation, encrypted content, forced RLS, durable bounded worker completion, a fixed stateless OpenAI adapter, integer reservation/usage reconciliation, ordered fail-closed tenant lifecycle projection, ADR-0028 erasure participation, the validated BFF/browser journey, provider safety/telemetry/cancellation, activation egress, signed v3.1 evaluation/provider-approval/canary receipts, and an exercised one-percent synthetic staging canary with rollback. Runtime returned to safe-disabled `false/0`; later canary steps and Production remain separately gated. | 9 |
...
| 8 | `3e58e6e1838721f798365349db3a832c3a3bcbd8` | `b85f112c83f41d06e93151dee300c2d82ab8eece` | `COMPLETED`: the complete Stage 8 diff was reviewed against unchanged `origin/main` with no remaining HR-015/HR-016 architecture-gate finding. ADR-0057, versioned model/prompt/price/eval assets, bilingual synthetic adversarial cases, fail-closed provider-data controls, safety/feedback/drift/canary/rollback policy, JSON consumer schemas, prompt digest, and an eight-test standard-library semantic gate are present. `make context-verify`, `make context-bootstrap`, `make mlops-verify`, `make script-static-verify`, and `make baseline-verify` passed locally. Protected repository baseline run `34684052338` and frontend E2E run `34684052271` passed at the reviewed head. No provider credential/call, model evaluation/execution, Conversation service, runtime canary, or Production readiness is claimed. |
| 9 | `9f0a811f78006f76c25d95b4499d4dfc868ba5aa` | `39ad9a8d41539ddff01df27f8e3b05a3cf258552` | `COMPLETED`: the complete Stage 9 implementation/tooling diff and current authorities were reviewed. Conversation service, contracts, forced-RLS/encrypted persistence, one-attempt Authorization/provider edges, durable worker/cost/ambiguity behavior, lifecycle/erasure, BFF/UI, fixed egress, telemetry, and deterministic rollout are implemented. Protected baseline run `36000377069` and frontend E2E run `36000377338` passed at `39ad9a8`. Exact canary runtime `afb738d6623316e876ed938e0869bba39b8a21a7` and image digest `sha256:4e08858bb68042d47d29623fd7d2421c3bdf687d0e571c329f1a8d78dc13e909` passed one synthetic one-percent run, 206-minute observation, privacy/log checks, rollback, and post-disable negative proof. Private mode-0600 signed receipts retain content-free provider/evaluation/canary evidence; runtime is disabled and Production remains `NOT VERIFIED`. |
...
#### Stage 8 final review report

| Required field | Review evidence |
| --- | --- |
| Architecture review mode | `full-read`; Core AI/product, external-provider, privacy, security, cost, reliability, and MLOps governance reviewed under critical minimal-safe priorities; the optional packaged skill was unavailable, so the repository rules were applied directly |
| Architecture document version/commit | Current authorities and complete Stage 8 diff reviewed at `b85f112c83f41d06e93151dee300c2d82ab8eece`; latest `origin/main` remained `3e58e6e1838721f798365349db3a832c3a3bcbd8` |
| Architecture sections reviewed | Mandatory source order; ADR-0054; Conversation/BFF/Authorization; security/threat/data/reliability/performance/testing/technology/readiness/status authorities; current implementation, contracts, tests, Git history/status/diff |
| Search terms used | `Stage 8`, `HR-015`, `HR-016`, `MLOps`, `model`, `prompt`, `price`, `eval`, `safety`, `feedback`, `drift`, `store`, `background`, `tools`, `PII`, `cost`, `canary`, `rollback` |
| External/version-sensitive sources | Context7 resolved the high-reputation official OpenAI API docs and queried Responses stateless behavior, data retention controls, and safety/evaluation guidance; the current official GPT-5.4 model page supplied the exact dated candidate snapshot and captured price |
| ADRs reviewed or changed | ADR-0013/0025/0028/0044/0045/0054 reviewed; ADR-0057 added and registered as current authority |
| Changed bounded context/module | Git-owned `mlops/` governance assets and standard-library CI verifier added; Conversation architecture refined; no new deployable, service source, database, provider credential, or runtime edge |
| Contracts changed | Added versioned JSON consumer schemas, catalog/eval bundle, exact prompt digest, and twelve synthetic bilingual examples; no public OpenAPI or inter-service Protobuf change |
| Database migration | `Not applicable`; no database or migration changed |
| Transaction boundary | `Not applicable`; no executable application transaction changed; ADR-0054 no-remote-I/O-in-transaction boundary preserved |
| Timeout/deadline behavior | No runtime call added; future provider one-attempt 60-second hard deadline and 20s/45s p95/p99 promotion thresholds are machine-checked |
| Retry/cancellation/concurrency behavior | No runtime behavior changed; ADR-0054 one attempt/no blind retry, bounded canary, cancellation ambiguity, and rollback/disable policy preserved |
| Kafka/event and idempotency behavior | `Not applicable`; no event/contract/runtime change; ADR-0028 future Conversation participation remains required before content enablement |
| Security impact | Model output remains untrusted/no-authority/no-side-effect; tool/MCP/web/code/action capabilities stay blocked; provider execution fails closed pending account control approval; prompt path traversal/tampering, weakened critical threshold, premature enablement, malformed policy/price, and direct contact PII fixtures are rejected |
| Authentication/authorization/tenant impact | No auth contract or permission changed; caller cannot select provider/model/prompt/canary; future feedback remains tenant/erasure scoped |
| Istio identity and authorization impact | `Not applicable`; no workload or network policy changed; fixed provider egress remains a Stage 9 implementation gate |
| Logging and PII impact | Evaluation fixtures are synthetic non-PII; production content is not automatic eval/training input; prompts/outputs/provider payload/subject identifiers remain prohibited telemetry; future receipts are content-free |
| Observability added or changed | Governance defines low-cardinality quality/safety/error/latency/cost/drift signals and rollback triggers; no runtime telemetry was claimed or added |
| Build/CI/architecture enforcement changed | `make mlops-verify` with eight positive/negative tests and semantic cross-file validation is now a mandatory `make baseline-verify` prerequisite; FILE_INDEX, ADR register, Context route/matrix, source index, and baseline required paths enforce presence |
| Tests executed | `Passed`: `make context-verify`, `make context-bootstrap`, `make mlops-verify`, Python compile, `make script-static-verify`, `make baseline-verify`, diff/index/matrix checks; protected baseline `34684052338` and frontend E2E `34684052271` passed at exact head `b85f112` |
| Architecture deviations | None identified. Stage 8 intentionally does not fabricate real model evaluation, provider-account approval, canary, or runtime evidence; those fail-closed Stage 9/10 gates remain `NOT VERIFIED` |
| Rollback considerations | Invalid/missing governance disables execution; never select an alias/default silently. Preserve immutable versions/receipts, prompt/content privacy, conservative cost/ambiguity, erasure, and every ADR-0054 excluded capability |

#### Stage 9 final review report

| Required field | Review evidence |
| --- | --- |
| Architecture review mode | `full-read`; Conversation, MLOps/provider, security/privacy, cost, persistence, messaging, BFF/browser, staging platform, reliability, testing, and Production separation reviewed under critical minimal-safe priorities |
| Architecture document version/commit | Stage 9 base `9f0a811f78006f76c25d95b4499d4dfc868ba5aa`; exact canary runtime `afb738d6623316e876ed938e0869bba39b8a21a7`; final receipt tooling/protected evidence head `39ad9a8d41539ddff01df27f8e3b05a3cf258552`; latest reviewed `origin/main` was `897093f18851704753a66cf8624dba87feaffd31` before final merge |
| Architecture sections reviewed | Mandatory source order; ADR-0013/0025/0028/0044/0045/0054/0057; Conversation, BFF, Authorization, data/messaging, security/threat, reliability/performance/testing, MLOps, staging/Production readiness, current implementation/contracts/tests/diff |
| External/version-sensitive sources | Context7 official OpenAI API reference plus current official OpenAI GPT-5.4 documentation; exact snapshot access was separately verified without paid inference before the single canary |
| ADRs reviewed or changed | ADR-0054/0057 and supporting current ADRs reviewed; no new ADR or architectural deviation required |
| Changed bounded context/module | Conversation service and its BFF/browser/contract/deployment/evidence boundary; Git-owned MLOps receipt tooling; no new deployable or speculative Agent/Workflow/tool/RAG boundary |
| Contracts changed | Versioned validated example-backed private gRPC and BFF OpenAPI/generated consumer types are implemented; canary receipt v1 JSON schema is content-free and machine-verified |
| Database migration | Six immutable Conversation Flyway migrations and forced-RLS/service-role gates passed; no executed migration was edited by final canary/receipt completion |
| Transaction boundary | Provider and Authorization I/O remain outside DB transactions/locks; state/reservation/message/queue and completion/cost effects retain atomic boundaries |
| Timeout/deadline behavior | One 300ms Authorization attempt, one bounded provider attempt with 60s maximum, bounded BFF calls/polling, and finite worker leases remain enforced |
| Retry/cancellation/concurrency behavior | No layered/blind provider retry; bounded global/tenant concurrency; cancellation and ambiguous outcome remain conservative; post-canary disable rejects before provider I/O |
| Kafka/event and idempotency behavior | Transactional Outbox/atomic Inbox, five-participant erasure, ordered lifecycle projection, UUIDv4 replay identity, and restore/reconciliation evidence passed |
| Security impact | Exact JWT audience/workload identity/permission checks, forced RLS, encrypted content, secret files mode 0600, fixed egress, no tools/provider URL selection, and fail-closed rollout preserved |
| Authentication/authorization/tenant impact | Synthetic canary tenant received only four exact Conversation permission overrides; all synthetic tenant/application authority and data were deleted after the signed aggregate receipt was issued |
| Istio identity and authorization impact | Exact Web BFF workload identity traversed the Ambient waypoint to Conversation; conditional provider egress was enabled only for the canary and normal false/zero deployment restored |
| Logging and PII impact | Prompt/output/credential and User/Tenant/Membership/Conversation/Run identifiers were absent from service logs; signed receipt contains only aggregate/version/digest evidence and no content/subject identifiers |
| Observability added or changed | Low-cardinality provider/run/cost/latency signals, alert/dashboard/runbook, leak checks, and rollback state were exercised; telemetry remained non-authoritative |
| Build/CI/architecture enforcement changed | Canary receipt schema/issuer/verifier and negative tests added; `make mlops-verify`, script static checks, baseline, service security suites, frontend E2E, staging, and provenance gates cover the slice |
| Tests executed | Conversation full Gradle suite passed locally; repository baseline run `36000377069` and frontend E2E run `36000377338` passed at `39ad9a8`; one paid canary only, actual cost 1210 micro-USD; 206-minute observation and rollback negative passed |
| Architecture deviations | None identified. The Git catalog and deployed runtime remain disabled after evidence; Stage 9 completion does not imply later canary or Production approval |
| Rollback considerations | Preserve content-free receipts and immutable governance. Any expired/mismatched evidence, critical incident, threshold breach, or control change keeps runtime false/zero until a fresh reviewed authorization and observation pass |
