...
| 3 | Frontend resilience and privacy | `NEXT` | Add one bounded abortable BFF request boundary, consistent safe problem mapping, busy/double-submit/cancellation/error states, error boundary, safe storage failure behavior, minimal persisted state, and prompt PII clearing. | Pending |
