...
| HR-015 | HIGH | MLOps governance | `CLOSED FOR STAGE 8`: ADR-0057 and the machine-validated Git assets now define a versioned offline synthetic/adversarial suite, quality/safety/cost/latency promotion thresholds, canary policy, and model/prompt rollback contract. Real model evaluation/canary execution remains correctly gated for Stage 9. | 8 |
| HR-016 | HIGH | AI safety/data control | `CLOSED FOR STAGE 8`: Conversation owns safety/acceptable-use decisions; provider-account data-control approval fails closed; enum-only feedback, drift triggers, non-PII evaluation handling, and no automatic production-content training are explicit and validated. Environment approval/execution remains Stage 9/10 evidence. | 8 |
| HR-017 | HIGH | Core product | `IN PROGRESS`: the neutral contract and executable Conversation service now include Authorization/private CRUD/history, idempotent ModelRun acceptance/cancellation, encrypted content, forced RLS, durable bounded worker completion, a fixed stateless OpenAI adapter, integer reservation/usage reconciliation, ordered fail-closed tenant lifecycle projection, rollout-gated ADR-0028 erasure participation, and the validated BFF/browser journey. Provider safety/evaluation/telemetry and cancellation propagation, activation egress, release integration, and runtime evidence remain. | 9 |
...
| 9 | ADR-0054 private Conversation + ModelRun vertical slice | `IN PROGRESS` | Implement the accepted service/contracts/DB/RLS/encryption/worker/provider/cost/lifecycle/telemetry/BFF/UI slice and its security/privacy/failure/load/browser/Helm evidence, preserving every ADR-0054/0057 exclusion and activation gate. | Neutral contracts, executable service foundation, authority boundary, private Conversation CRUD/lifecycle, encrypted Message history, exact governed model/prompt/price selection, idempotent ModelRun acceptance/get/cancellation, atomic tenant/Membership reservation, durable metadata-only `SKIP LOCKED` worker leases, fixed one-attempt stateless OpenAI adapter, completion/cancellation/unknown-outcome handling, integer actual-cost reconciliation, ordered fail-closed tenant lifecycle projection, rollout-gated ADR-0028 erasure participation, and validated OpenAPI/BFF/browser journey are implemented in the active slice. Current governance and cluster egress keep execution disabled. Local strict service, contract, Helm, component, and browser checks pass for the implemented slices; protected evidence remains commit-specific. PR #134 protected baseline run `35466974425` and frontend E2E run `35466974204` passed; post-merge baseline run `35467376618` and frontend E2E run `35467376506` passed at exact merge `2f450003fa50f0e83ce3321799d922e2467e4d92`. Provider safety/evaluation/telemetry and transport cancellation, activation egress, release and deployed-runtime evidence remain pending. |
