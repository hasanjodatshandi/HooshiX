...
The implemented service security suites install digest-verified OSV-Scanner 2.4.0 and scan locked Gradle dependencies for known vulnerabilities. Because the repository baseline invokes all five reusable service security suites on schedule, the same locked-dependency advisory scans also run on the scheduled repository security cadence. The Identity and Compromised Password suites configure immutable-digest Gitleaks 8.30.0 with mandatory positive detection controls plus redacted current-tree and full-Git-history scans for the repository. Current Compromised Password local fixture, current-tree, and full-history execution passes; protected evidence remains commit-specific until the workflow executes on the PR. Protected PR verification passed for the authentication/session/JWT source head `159d551`, and the post-merge `Repository baseline` run `32210656621` passed the Compromised Password, Notification, and Identity security suites plus the final baseline aggregator on `main@a3766bd` on 2026-08-19. The expanded five-service protected PR baseline run `32261626399` also passed on implementation head `7de8b17` on 2026-08-19. This is repository/early dependency-advisory evidence only; it is not final-image/SBOM vulnerability or deployed-runtime evidence.
...
- Gitleaks 8.30.0 redacted current-tree and full-Git-history scanning plus negative/current-tree-positive/commit-then-delete history fixtures is PRESENT in the Identity and Compromised Password security workflows; current Compromised Password local execution passed the fixtures, current tree, and full current Git history, while prior protected Identity tree/history execution passed on `main@a3766bd`; protected evidence remains commit-specific;
...
| --- | --- | --- | --- | --- | --- |
| Identity Service | DESIGNED | IMPLEMENTED registration + local-password authentication/Session/RefreshFamily/JWT + Tenant/Membership/Invitation/selection repository slices | current local revision: canonical WSL Java 25 strict Gradle + PostgreSQL/Redis Testcontainers, non-owner forced-RLS/pool-reuse negatives, four Flyway migrations, Buf compatibility, Semgrep, OSV, Gitleaks, Helm/render, Prometheus/dashboard, and digest-pinned runtime-image checks pass; the current V4 revision is redeployed in the local production-fidelity kind/staging lane with the exact-digest five-service stack and its Authorization caller path exercised through Ambient workload identity; protected CI remains PR-head-specific evidence; production runtime remains NOT VERIFIED | NOT VERIFIED | `services/identity-service` |
| Authorization Service | DESIGNED | IMPLEMENTED current repository slice | current local revision: canonical WSL Java 25 strict Gradle + PostgreSQL/Redis Testcontainers, non-owner forced-RLS/pool-reuse negatives, two Flyway migrations, bounded global/per-caller `CheckPermission` admission with <=25ms queue wait, Identity/resource caller binding through the Authorization waypoint, audit-failure/owner-reservation telemetry, ADR-0032 service-side burn rules, Buf compatibility, Semgrep, OSV, Gitleaks, exact Helm 4.2.4 render, Prometheus/dashboard, and digest-pinned runtime-image construction pass; the current V2/waypoint revision is redeployed in the local production-fidelity kind/staging lane with exact-digest five-service deployment, Flyway V2, waypoint positive caller binding, spoof/missing-caller RBAC denial, STRICT mTLS/workload identity, persistence isolation, edge, and observability verification passing; protected CI remains PR-head-specific evidence; production runtime remains NOT VERIFIED | NOT VERIFIED | `services/authorization-service` |
...
| Gitleaks current-tree/Git-history secret scanning | DESIGNED under ADR-0045 | PARTIAL | immutable-digest Gitleaks 8.30.0 Identity and Compromised Password workflows include negative/current-tree-positive/commit-then-delete fixtures and reviewed narrow false-positive policy; current Compromised Password local fixtures/current-tree/full-current-history passed, and prior protected Identity current-tree/full-history execution passed on `main@a3766bd`; Notification adoption remains outstanding and protected evidence remains commit-specific |
...
ADR-0045 documents additional target gates. Identity and Compromised Password Gitleaks workflows are implemented; Identity has prior local plus protected merged-main execution evidence on `main@a3766bd`, and current Compromised Password local fixture/tree/history execution passes while protected evidence remains commit-specific. Syft/Grype/Cosign/Kyverno must not be reported as implemented until their repository workflows/policies exist and execute successfully. OSV-Scanner must not be reported as final-image vulnerability evidence.
