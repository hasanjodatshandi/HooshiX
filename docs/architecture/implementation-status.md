...
The neutral Protobuf contract artifact is version 1.6.0. It includes versioned Protovalidate
...
Authorization and Web BFF application services are implemented as current repository slices. BFF OpenAPI 3.1 version 1.4.0 covers all 48 implemented public controller method/path mappings, including Google OIDC/ExternalIdentity, Identity MFA, cookie-bound MFA completion, and same-origin CSRF recovery, with schema validation, consumer examples, controller/OpenAPI parity, and generated frontend transport-type drift enforcement; executed checks remain change-specific PR/CI evidence and are not inferred by this status summary. Web BFF also provides trusted-client-address forwarding, RFC 9457 public errors, encrypted server-side pre-auth/MFA custody, provider-token isolation, semantic quota, and authoritative-security Identity edges. Identity has the registration, local-password/Google-evidence authentication, ExternalIdentity, Session/RefreshFamily/JWT-signing, MFA, and Tenant/Membership/Invitation/selection slices described above. Notification includes its delivery runtime and Liara/IPPanel adapter implementation, while real production provider execution remains NOT VERIFIED. The local kind production-fidelity evidence predates this OIDC revision. No production platform runtime, real Google/Liara/IPPanel provider execution, production provider/corpus approval, restore exercise, complete-stack load test, executed production artifact-signing release, executed production admission chain, or production traffic readiness is claimed.

## Capability/service status

| Capability | Architecture | Independent implementation | Runtime evidence | Production readiness | Planned target |
| --- | --- | --- | --- | --- | --- |
| Identity Service | DESIGNED | IMPLEMENTED registration + local-password/Google-evidence authentication + ExternalIdentity + Session/RefreshFamily/JWT + password/Profile/Contact + TOTP MFA/recovery-code lifecycle + Tenant/Membership/Invitation/selection repository slices | current local revision: canonical WSL Java 25 strict Gradle + PostgreSQL/Redis Testcontainers, eleven Flyway migrations through V11 ExternalIdentity/evidence replay state, issuer+subject/email-collision/replay/session/MFA negatives, encrypted replay result and TOTP state, bounded ExternalIdentity/MFA telemetry/alerts/dashboard, and existing password/Profile/Contact/Tenant evidence; protected CI and deployed Google/password/Profile/MFA runtime remain change/environment-specific and NOT VERIFIED until executed | NOT VERIFIED | `services/identity-service` |
