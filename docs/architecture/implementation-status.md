...
| --- | --- | --- | --- | --- | --- |
| Identity Service | DESIGNED | IMPLEMENTED registration + local-password authentication/Session/RefreshFamily/JWT + Tenant/Membership/Invitation/selection repository slices | current local revision: canonical WSL Java 25 strict Gradle + PostgreSQL/Redis Testcontainers, non-owner forced-RLS/pool-reuse negatives, six Flyway migrations including V5 tenant/session Membership identity constraints and V6 Notification result correlation, immutable key-ID refresh checks, bounded global zero-queue gRPC admission, Tenant lifecycle metrics, Notification terminal-result callback persistence, Buf compatibility, Semgrep, OSV, Gitleaks, Helm/render, Prometheus/dashboard, and digest-pinned runtime-image checks pass; current local integration evidence includes the five-service WSL application lane and existing production-fidelity kind/staging evidence; protected CI remains PR-head-specific; production runtime remains NOT VERIFIED | NOT VERIFIED | `services/identity-service` |
...
| Web BFF | DESIGNED | IMPLEMENTED current repository slice including Public Registration + OpenAPI | canonical WSL Java 25 strict Gradle + Redis Testcontainers gates pass; OpenAPI 3.1 contract, RFC 9457 registration problems, anonymous same-origin register/resend security, session-bound CSRF, trusted client-address forwarding, and BFF -> Identity register/resend/confirm transport contracts are regression-tested; the latest integrated WSL execution brought all five services Ready and live public register reached durable Identity state plus durable Notification `ACCEPTED`, while live resend created a fresh active challenge and submitted Notification outbox; confirm is verified through BFF -> Identity transport plus Identity confirmation tests without extracting encrypted OTP material; protected CI remains commit-specific; production edge/runtime remains NOT VERIFIED | NOT VERIFIED | `services/web-bff` |
