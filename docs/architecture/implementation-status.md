...
The implemented service security suites install digest-verified OSV-Scanner 2.4.0 and scan locked Gradle dependencies for known vulnerabilities. Because the repository baseline invokes all three reusable service security suites on schedule, the same locked-dependency advisory scans also run on the scheduled repository security cadence. The Identity suite configures immutable-digest Gitleaks 8.30.0 with a mandatory positive detection control plus redacted current-tree and full-Git-history scans for the repository. Protected PR verification passed for the authentication/session/JWT source head `159d551`, and the post-merge `Repository baseline` run `32210656621` passed the Compromised Password, Notification, and Identity security suites plus the final baseline aggregator on `main@a3766bd` on 2026-08-19. This is repository/early dependency-advisory evidence only; it is not final-image/SBOM vulnerability or deployed-runtime evidence.
...
| --- | --- | --- | --- | --- | --- |
| Identity Service | DESIGNED | IMPLEMENTED registration + local-password authentication/Session/RefreshFamily/JWT-signing repository slices | protected PR and merged-main CI for the current registration/authentication/session/JWT source is VERIFIED on `main@a3766bd`; deployed runtime, Tenant/Membership/MFA/OIDC/password-change/recovery/erasure and wider ADR-0012 behavior remain NOT VERIFIED | NOT VERIFIED | `services/identity-service` |
