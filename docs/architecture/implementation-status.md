...
ADR-0048 defines the separate developer-host Ops MCP. Under ADR-0051, its implementation, schemas, and tests are owned by the independent Windows MCP runtime. Runtime PR #1 added `process.start`, `process.status`, `process.logs`, and `process.cancel`; PR #2 closed the fail-closed state-path reread found during final review; PR #3 added `scripts/ops/job_cli.py` as a stale-client compatibility path that delegates to the same persistent-job engine through the already-established `process.run`/policy-`python` authority; PR #4 made that compatibility CLI emit explicit UTF-8 bytes so Unicode job output does not depend on the Windows console code page. Their authoritative merge commits are `03e4516ddbbc6b671d2e29043bf40172aa1daeca`, `3be8d1d723d95691bccc978034505940d90473af`, `0cacc1cd829df0f0cd3ca7636d3c4a76815904b7`, and `86daa41739e128dcebd91e1810a8524300f08ac9`. GitHub reported an empty `statusCheckRollup` for all four runtime PRs, so runtime evidence is the executed local/security/host verification, not a CI-green claim. The live elevated Ops policy retains the finite 300-second local command ceiling and the Context/Ops wrappers retain the one-hour MCP connection-TTL request. Live `ops.status` reports persistent bounds of 4 active jobs, 16 retained records, 24-hour cleanup age, 1 MiB maximum per persistent stdout/stderr stream, and 64 KiB maximum per log page. Direct live MCP discovery returned the reviewed 13-tool Ops surface; loopback `/healthz` and `/readyz` returned 200. First-class persistent-job evidence includes a WSL job that completed after `135127 ms` with exit code 0 while observation used short polling calls, plus a separate job that reached terminal `cancelled` through job-ID-only cancellation. The stale-discovery compatibility path was then exercised from an already-open client that still exposed the earlier nine-tool schema: the short `process.run` wrapper started a persistent WSL job that completed after `135158 ms` with exit code 0 and bounded marker output, and a separate compatibility job reached terminal `cancelled` by runtime-created job ID. This proves stale client discovery no longer requires the long target command to remain coupled to one synchronous response. The compatibility path is not a new execution authority and remains secondary to refreshed first-class tool discovery. Protected Ops/job-state ACLs were inspected and no broad `Everyone` grant was observed. Earlier synchronous evidence still shows a shorter tunnel/control-plane response lifetime, including one exercised failure at about `122603 ms`; persistent-job completion therefore proves decoupled local execution, not a longer synchronous response SLA.
...
The implemented service security suites install digest-verified OSV-Scanner 2.4.0 and scan locked Gradle dependencies for known vulnerabilities. Because the repository baseline invokes all five reusable service security suites on schedule, the same locked-dependency advisory scans also run on the scheduled repository security cadence. The Identity suite configures immutable-digest Gitleaks 8.30.0 with a mandatory positive detection control plus redacted current-tree and full-Git-history scans for the repository. Local developer-host tool availability was reverified on 2026-08-20: the WSL Gitleaks wrapper resolves to the reviewed 8.30.0 immutable image and a full-history scan covered 464 commits / about 14.08 MB with no leaks found; the repository-pinned Semgrep image ran the Authorization rules against 39 tracked Java files with 7 rules and 0 findings; the OSV-Scanner 2.4.0 Linux/x64 binary matched repository SHA-256 `15314940c10d26af9c6649f150b8a47c1262e8fc7e17b1d1029b0e479e8ed8a0` and scanned the Authorization Gradle lockfile as 230 packages with no issues found. These are local fast-feedback checks only. Protected PR verification passed for the authentication/session/JWT source head `159d551`, and the post-merge `Repository baseline` run `32210656621` passed the Compromised Password, Notification, and Identity security suites plus the final baseline aggregator on `main@a3766bd` on 2026-08-19. The expanded five-service protected PR baseline run `32261626399` also passed on implementation head `7de8b17` on 2026-08-19. This is repository/early dependency-advisory evidence only; it is not final-image/SBOM vulnerability or deployed-runtime evidence.
...
- ADR-0048/0051 Ops source/schema/tests are external in `hasanjodatshandi/HooshiXMcpRuntime`; current runtime `main@86daa41739e128dcebd91e1810a8524300f08ac9` passes Context 14/14, Ops 36/36, and Desktop 60/60; live 13-tool discovery, first-class 135-second persistent execution, stale-discovery compatibility execution beyond one response window, runner-owned cancellation, explicit UTF-8 compatibility output, ACL, health/readiness, and audit-redaction evidence passed;
...
| --- | --- | --- | --- | --- | --- |
| Identity Service | DESIGNED | IMPLEMENTED registration + local-password authentication/Session/RefreshFamily/JWT + Tenant/Membership/Invitation/selection repository slices | current local revision: canonical WSL Java 25 strict Gradle + PostgreSQL/Redis Testcontainers, non-owner forced-RLS/pool-reuse negatives, four Flyway migrations, Buf compatibility, Semgrep, OSV, Gitleaks, Helm/render, Prometheus/dashboard, and digest-pinned runtime-image checks pass; protected CI remains commit-specific to earlier reviewed heads; the fully enabled Identity runtime and production-fidelity kind/staging lane have earlier commit-specific integration evidence, but this V4 migration revision has not been redeployed through those heavy lanes; production runtime remains NOT VERIFIED | NOT VERIFIED | `services/identity-service` |
| Authorization Service | DESIGNED | IMPLEMENTED current repository slice | canonical WSL Java 25 strict Gradle + PostgreSQL/Redis Testcontainers gates pass; the real migration profile applies the current Flyway migration; Authorization is runtime-enabled and `UP`/Ready simultaneously with the other four services against its non-owner local DB role and local Redis/key/JWT-verifier fixtures; protected CI remains commit-specific; the local production-fidelity kind staging deployment is Ready and passes the composite staging verifier; production runtime remains NOT VERIFIED | NOT VERIFIED | `services/authorization-service` |
| Notification Service | DESIGNED | IMPLEMENTED | canonical WSL Java 25 strict Gradle + PostgreSQL Testcontainers gates pass; the real migration profile applies both current Flyway migrations; Notification is `UP`/Ready simultaneously with the other four services using its local-only simulated providers and non-owner local DB role; repository CI evidence remains commit-specific; the local production-fidelity kind staging deployment is Ready with non-production provider configuration; production Liara/IPPanel provider/runtime evidence NOT VERIFIED | NOT VERIFIED | `services/notification-service` |
...
| Compromised Password Service | DESIGNED | IMPLEMENTED | canonical WSL Java 25 strict Gradle/integration/bootJar gates pass; the repository-built `GENERATED_TEST_FIXTURE` dataset runtime is `UP`/Ready simultaneously with the other four services in the local integrated runtime; repository CI evidence remains commit-specific; the local production-fidelity kind staging deployment is Ready with an exact-digest-bound `GENERATED_TEST_FIXTURE`; production HIBP corpus/runtime evidence NOT VERIFIED | NOT VERIFIED | `services/compromised-password-service` |
...
| ChatGPT Web developer-host Ops MCP | DESIGNED under ADR-0048/0051 | IMPLEMENTED external Windows runtime + host integration | runtime `main@86daa417` passes Context 14/14, Ops 36/36, Desktop 60/60; live Ops discovery has 13 reviewed tools; local timeout remains 300s and wrappers request 1h MCP connection TTL; first-class persistent execution completed after 135127ms; the stale nine-tool client schema also completed a 135158ms persistent WSL job through the compatibility CLI and job-ID-only cancellation reached `cancelled`; compatibility JSON output is explicit UTF-8; synchronous tunnel lifetime remains a separate shorter bound and is not extended by persistent jobs; production authority NOT APPLICABLE |
