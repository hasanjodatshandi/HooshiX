...
approval. The neutral Conversation transport contract and permission catalog are present, while
`services/conversation-service/` is not yet present at this revision; no provider credential, model
execution, Conversation database, public route, or runtime evidence is claimed.
