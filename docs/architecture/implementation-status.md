...
ADR-0047 adds repository-side readiness for ChatGPT Web to reach that existing stdio MCP adapter through the external OpenAI Secure MCP Tunnel bridge. Repository evidence includes the CWD-independent MCP entry point, unchanged five-tool read-only contract, modern positive-search MCP coverage, structured + serialized JSON tool-result compatibility, reviewed tunnel-client v0.0.11 developer pin/integrity metadata, least-privilege credential rules, threat/security governance, and Windows operator runbook. HooshiX still adds no HTTP/SSE/TCP MCP listener or public MCP port. Actual installation on the operator PC, runtime API key permissions, tunnel-client `/readyz`, current custom-app tool discovery/action state, startup/reboot persistence, remote-main freshness at runtime, and real ChatGPT Web `project.bootstrap` plus positive-result `project.search` calls are **NOT VERIFIED** until executed in that environment.
...
- ADR-0047 ChatGPT Web tunnel repository integration artifacts are present; real operator-PC/ChatGPT tunnel runtime/freshness/custom-app evidence is NOT VERIFIED;
...
| ChatGPT Web Context Engine tunnel bridge | DESIGNED under ADR-0047 | IMPLEMENTED repository integration | CWD-independent stdio entry point + fixed five read-only tools + positive-search result compatibility tests + v0.0.11 developer pin/runbook/security governance present; operator PC tunnel-client/key/readiness/freshness/custom-app/Web calls NOT VERIFIED |
...
- Context Engine retrieval remains tracked-file/local/bounded/provenance-bearing and excludes configured sensitive filename/path patterns;
- Context MCP remains a fixed read-only/stdio-only five-tool adapter, starts independently of caller working directory, and now tests positive modern search result compatibility;
- the baseline does **not** by itself prove the external tunnel-client process, operator-PC credential scope, remote `origin/main` freshness, ChatGPT custom-app configuration, or absence of every possible separately-added network wrapper; those remain operator/integration evidence plus architecture/diff-review obligations under ADR-0047;
...
- actual ADR-0047 operator-PC tunnel-client installation, restricted runtime API key permission proof, healthy `/readyz`, explicit `HEAD == origin/main` freshness evidence, ChatGPT custom-app tool discovery/action state, startup/reboot persistence, and ChatGPT Web `project.bootstrap`/routing/positive-result `project.search` calls;
