...
The v2 candidate's signed content-free real-provider evaluation passed 12/12, including 8/8 critical
cases with zero errors and $0.033958 total measured cost; execution remains disabled pending
provider-account approval and canary/rollback evidence. The neutral Conversation transport contract
...
`CANARY_1/5/25/APPROVED_100` mechanics are implemented. Real provider evaluation passed for the v2
tuple. Provider-account data-control approval, retained approval references, deployed canary observation and
...
| Web BFF | DESIGNED | IMPLEMENTED public facade plus ADR-0028 session-state participant | OpenAPI 3.1 version 2.1.0 covers all 67 public operations including self-erasure and Conversation and separates UUIDv4 `Idempotency-Key` business identity from mutable `X-Request-Id` telemetry; Conversation uses exact audience/workload policy, one 900ms attempt, bounded concurrency, and fail-closed safe mapping; schema/examples/parity/generated-type drift, PostgreSQL V1 participant state, Redis indexed-session erasure, atomic Inbox/receipt Outbox, strict full check, Helm/Prometheus and local Kafka smoke pass locally; deployed Production evidence remains NOT VERIFIED | NOT VERIFIED | `services/web-bff` |
...
| Conversation Service | DESIGNED under ADR-0054/0057 | PARTIAL: executable service, authority, CRUD/history, ModelRun/cost, lifecycle/erasure, BFF/browser, provider boundary, and offline evaluation slices IMPLEMENTED; activation pending | Local evidence covers exact-audience JWT, one-attempt Authorization, validated gRPC/OpenAPI, ownership/RLS/encryption, reservation/replay/cancellation, bounded worker/provider, conservative ambiguity and reconciliation, ordered lifecycle/erasure, exact BFF workload routing, browser polling/text rendering, and a passing signed v2 real-provider evaluation. Governance and cluster egress keep product execution disabled; provider-account controls, owner approval references, deployed canary/rollback, and product runtime remain NOT VERIFIED | NOT VERIFIED | `services/conversation-service`; `mlops/` |
...
| AI model evaluation/safety governance | DESIGNED under ADR-0057 | IMPLEMENTED as versioned Git-owned catalogs/schemas/prompt/synthetic suite, deterministic gate, and signed content-free provider runner; runtime disabled | v2 real-provider evaluation at `bc5af2853ed08d5d4f4bbd39f77bda3feac49f2c` passed 12/12, 8/8 critical, zero errors, p95 4.986s, and $0.033958 total measured cost. Account data-control approval, owner approval references, canary/rollback execution, and model product runtime remain NOT VERIFIED |
