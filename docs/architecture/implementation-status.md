...
The Identity service repository implementation includes the local EMAIL/PHONE registration contract and application flow, profile/contact canonicalization, PENDING-to-ACTIVE verification lifecycle, globally unique verified-contact persistence primitives, one-live-contact reservation, eight-digit HMAC-only registration challenges, versioned HMAC idempotency, Argon2id credential hashing, fail-closed Compromised Password screening, ADR-0024 registration and LOGIN Redis semantic-quota controls, encrypted durable Notification handoff, PostgreSQL/Flyway+jOOQ persistence, Day-One telemetry/readiness, hardened container/Helm/ServiceAccount/NetworkPolicy/Istio artifacts, and service-owned quality/security gates. The current source additionally implements local-password authentication through verified Contact, a 256-bit server-side Session/RefreshFamily model, digest-only rotating refresh credentials with seven-day idle/30-day absolute lifetime, deterministic 20-family eviction, predecessor-reuse family revocation, logout-current/logout-all, bounded session security-evidence retention, and local ADR-0023 RSA-3072/RS256 key validation/signing machinery. `IssueAudienceAccessToken` is present but the only implemented session mode is `authenticated_onboarding`, so ordinary tenant/resource JWT issuance fails closed until Tenant/Membership selection authority exists. Phone registration remains server-gated off by default. MFA, OIDC, Tenant/Membership lifecycle/selection, password change/recovery, erasure, deployed runtime, production key rotation, production quota thresholds, real host-time synchronization integration, load/recovery, and production readiness remain NOT VERIFIED. The current architecture also does not define a concrete password composition/minimum-length rule beyond NFC normalization, compromised-password screening, and the Argon2id storage baseline; production registration/password-policy readiness remains blocked until that security-policy gap is resolved rather than guessed in code.

The implemented service security suites install digest-verified OSV-Scanner 2.4.0 and scan locked Gradle dependencies for known vulnerabilities. Because the repository baseline invokes all three reusable service security suites on schedule, the same locked-dependency advisory scans also run on the scheduled repository security cadence. The Identity suite configures immutable-digest Gitleaks 8.30.0 with a mandatory positive detection control plus redacted current-tree and full-Git-history scans for the repository. Protected PR verification passed for the registration slice, and the merged-main repository workflow passed the Compromised Password, Notification, and Identity security suites plus the final baseline aggregator on `main@9642507` on 2026-08-18. This is repository/early dependency-advisory evidence only; it is not final-image/SBOM vulnerability or deployed-runtime evidence.
...
- ADR-0049/0050 Desktop MCP source/policy schema/tests/runbook are present; final protected-policy GUI smoke, separate tunnel/ChatGPT discovery/status, persistent interactive task, and tunnel/MCP/parent recovery evidence remains valid for ADR-0049; ADR-0050 repository broker tests plus EOrgsetad process-image/SHA-256/native-password-target diagnosis and policy-bound credential injection/login host smoke passed on the Medium-integrity `asInvoker` launch path; real logoff/logon, provisioned wrong-window/focus/ambiguous-password negatives, and revocation/rollback remain NOT VERIFIED;
- service-specific Semgrep enforcement exists for Compromised Password, Notification, and Identity; local Identity execution with the pinned image, local rules, network disabled, version check disabled, and metrics disabled completed with zero findings, and protected Semgrep execution passed for all three implemented service suites on `main@9642507`;
- OSV-Scanner locked-dependency advisory scanning exists for Compromised Password, Notification, and Identity and runs in PR/push/scheduled security verification; the merged-main locked-dependency advisory scans passed for all three implemented service suites on `main@9642507`;
- Gitleaks 8.30.0 redacted current-tree and full-Git-history scanning plus negative/current-tree-positive/commit-then-delete history fixtures is PRESENT in the required Identity security workflow; local execution passed the fixtures, current tree, and full current Git history with the reviewed narrow false-positive policy, and the same protected Identity gate passed on `main@9642507`;
...
Authorization and Web BFF application services remain absent. Identity has the registration plus local-password authentication/Session/RefreshFamily/JWT-signing repository slices described above, and Notification is implemented as a repository vertical slice, while production Liara/IPPanel provider integration remains absent. No production platform runtime, complete observability backend, restore exercise, complete-stack load test, artifact-signing release pipeline, final-image vulnerability/admission gate, or production traffic readiness is claimed.

## Capability/service status

| Capability | Architecture | Independent implementation | Runtime evidence | Production readiness | Planned target |
| --- | --- | --- | --- | --- | --- |
| Identity Service | DESIGNED | IMPLEMENTED registration + local-password authentication/Session/RefreshFamily/JWT-signing repository slices | protected registration-slice CI VERIFIED on `main@9642507`; current authentication/session slice has local source/integration/render evidence but protected commit-specific CI, deployed runtime, Tenant/Membership/MFA/OIDC and wider ADR-0012 behavior remain NOT VERIFIED | NOT VERIFIED | `services/identity-service` |
...
| ChatGPT Web developer-host Desktop MCP | DESIGNED under ADR-0049/0050 | IMPLEMENTED repository + host integration | ADR-0049 repository/host UI+tunnel evidence passed as recorded; ADR-0050 broker repository tests plus EOrgsetad process-image/native-password-target diagnosis and real policy-bound credential injection/login passed after relaunching its `asInvoker` application at Medium integrity; the initial inherited High-integrity launch remained correctly blocked by UIPI; real logoff/logon, provisioned wrong-window/focus/ambiguous-password negatives, and revocation/rollback NOT VERIFIED |
...
| Identity registration/authentication CI/architecture/security/migration/quota/deployment gates | DESIGNED | IMPLEMENTED for current repository slices | registration protected merged-main execution passed Gitleaks, OSV, strict Gradle verification, unit/integration/architecture/SpotBugs, Buf, Semgrep, Helm, Prometheus/dashboard, runtime-image, generated-file, and baseline gates on `main@9642507`; current local authentication/session/JWT evidence passed WSL Java 25 unit/contract plus executable Cucumber-JVM 7.34.6/Gherkin acceptance scenarios, PostgreSQL/Flyway + Redis Testcontainers integration, ArchUnit, SpotBugs main/test, Spotless, bootJar, Buf lint/build/breaking, pinned Semgrep, OSV-Scanner, Gitleaks current-tree, Helm enabled/disabled render hardening, Prometheus/dashboard validation, and pinned runtime-image construction; authentication commit-specific protected CI and deployed-runtime evidence remain NOT VERIFIED |
| Semgrep source SAST/policy | DESIGNED under ADR-0039/0045 | PARTIAL | implemented for Compromised Password, Notification, and Identity; pinned local Identity scan passed with zero findings after explicit offline version-check/metrics suppression; protected Semgrep execution passed for all three implemented service suites on `main@9642507`; future-service coverage remains implementation-dependent |
| Gitleaks current-tree/Git-history secret scanning | DESIGNED under ADR-0045 | PARTIAL | immutable-digest Gitleaks 8.30.0 Identity workflow includes negative/current-tree-positive/commit-then-delete fixtures and reviewed narrow false-positive policy; local fixtures/current-tree/full-current-history passed, and protected Identity current-tree/full-history execution passed on `main@9642507`; broader service adoption remains scope-dependent |
| OSV-Scanner declared/locked dependency advisory scan | DESIGNED under ADR-0045 | PARTIAL | OSV-Scanner 2.4.0 implemented for Compromised Password, Notification, and Identity PR/push/scheduled service-security suites; protected merged-main scans passed for all three implemented services on `main@9642507`; future-service coverage remains implementation-dependent |
