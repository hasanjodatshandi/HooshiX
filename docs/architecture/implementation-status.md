...
| Conversation Service | DESIGNED under ADR-0054/0057 | PARTIAL: executable foundation, authority boundary, private Conversation CRUD/lifecycle, encrypted USER Message history, and budgeted ModelRun acceptance/get/cancel repository slices IMPLEMENTED; product vertical slice pending | Local unit/integration evidence covers exact-audience JWT, one-attempt 300ms Authorization, validated/bounded gRPC, stable error mapping including budget exhaustion, Membership ownership, AES-GCM title/Message content, opaque pagination, optimistic versions, exact governed model/prompt/price policy, atomic tenant/Membership reservation, durable replay/conflict/cancellation semantics, delete-time Message purge/reservation release, PostgreSQL 18.4 Flyway/forced RLS/pool reuse, and key rotation. The complete strict service/security/repository gates for the active change remain pending until its PR receipt; worker/provider/completion and cost reconciliation, tenant lifecycle/ADR-0028 erasure, BFF/frontend, release and deployed-runtime evidence remain NOT VERIFIED | NOT VERIFIED | `services/conversation-service`; `mlops/` |
