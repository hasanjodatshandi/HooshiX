...
- OSV-Scanner locked-dependency advisory scanning exists for Compromised Password, Notification, and Identity and runs in PR/push/scheduled security verification; the pinned Identity OSV 2.4.0 locked-dependency scan passed locally on this branch;
...
- protected-CI execution evidence for the configured blocking Gitleaks current-tree + fail-fast Git-history scans, positive/negative fixtures, commit-then-delete fixture, and redacted output;
