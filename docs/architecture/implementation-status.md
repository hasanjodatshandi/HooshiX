...
- ADR-0049 Desktop MCP source/policy schema/tests/runbook are present; final protected-policy non-elevated Notepad host smoke passed exact mixed-case/`✓`/Persian Unicode typing, shortcut save, mouse/UIA, window/full-screen capture, audit/capture cleanup after the isolated Unicode-helper + WinApp chord fixes; separate Desktop tunnel/ChatGPT/persistent-task/revocation evidence remains NOT VERIFIED;
...
| ChatGPT Web developer-host Desktop MCP | DESIGNED under ADR-0049 | IMPLEMENTED repository integration + partial host smoke | repository tests present; final-policy non-elevated host smoke passed exact mixed-case/`✓`/Persian Unicode text, WinApp shortcut/UIA/mouse, window/full-screen screenshot, and cleanup after isolated Unicode-helper/chord fixes; separate tunnel/ChatGPT/persistent-logon/revocation evidence NOT VERIFIED |
