...
| Conversation Service | DESIGNED under ADR-0054/0057 | PARTIAL: executable foundation, authority boundary, and private Conversation CRUD/lifecycle repository slice IMPLEMENTED; product vertical slice pending | Local strict Gradle unit/integration/architecture/coverage/SpotBugs/JAR checks cover exact-audience JWT, one-attempt 300ms Authorization, validated/bounded gRPC, stable error mapping, Membership ownership, encrypted titles, opaque pagination, optimistic versions, durable request replay/conflict handling, PostgreSQL 18.4 Flyway/forced RLS/pool reuse, key rotation, Helm and observability for this slice. Message/ModelRun, provider, cost, tenant lifecycle/erasure, BFF/frontend, release and deployed-runtime evidence remain NOT VERIFIED | NOT VERIFIED | `services/conversation-service`; `mlops/` |
