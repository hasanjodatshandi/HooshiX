...
The repository baseline verifies file-index consistency, ADR/register coverage, dependency-registry/view consistency, current source references, guarded structure, ADR-0046 project Context Engine/bootstrap/routing/retrieval/checkpoint contracts, and the ADR-0051 externalized-MCP path guard. The independent Windows MCP runtime owns Context adapter, Ops, and Desktop runtime tests. The repository workflow still invokes all five implemented service security suites. Independent runtime commit `f978392b92a7132b312ba84f66cbe1eac9738eca` passed Context 14/14, Ops 21/21, and Desktop 60/60 unit/security tests during this migration.
...
ADR-0048 defines the separate developer-host Ops MCP. Under ADR-0051, its implementation, schemas, and tests are owned by the independent Windows MCP runtime. The live policy no longer allows the old Windows HooshiX checkout and uses an explicit `wsl.exe` alias for project commands. Migration smoke verified `git status --short` in `/home/coder/workspace/Hooshix`. On 2026-08-20 the live Ops policy was updated to a finite 300-second local command ceiling, the independent Context WSL adapter was updated to the same 300-second command bound, and the Context/Ops tunnel wrappers requested a one-hour MCP connection TTL. Context 14/14 and Ops 21/21 independent runtime tests passed. Host audit proved a local Ops command can remain active for `197968 ms` without the local timeout firing, but separate tunnel calls still lost their synchronous response when the effective tunnel/control-plane command lifetime expired, including one exercised failure at about `122603 ms`. The five-minute value is therefore verified as a local execution bound, not as a five-minute ChatGPT response SLA; longer work uses split/checkpoint or persistent-terminal/polling behavior. Earlier policy/tunnel/audit/recovery evidence remains host evidence.
...
- ADR-0048/0051 Ops source/schema/tests are external; the HooshiX runbook remains, external runtime tests passed, and the WSL project-command bridge passed migration smoke;
...
A developer-only local application-integration infrastructure is now present under `infrastructure/local/` with the WSL runtime supervisor under `scripts/local/runtime.py`. It runs the current five executable services together against pinned loopback PostgreSQL/Redis and Git-ignored local security/provider fixtures. Root `deploy/` and the production-fidelity/platform infrastructure for kind/Calico/Istio/Traefik/WAF/observability/GitOps remain absent or unverified.
...
| --- | --- | --- | --- | --- | --- |
| Identity Service | DESIGNED | IMPLEMENTED registration + local-password authentication/Session/RefreshFamily/JWT + Tenant/Membership/Invitation/selection repository slices | canonical WSL Java 25 strict Gradle + PostgreSQL/Redis Testcontainers gates pass; the real migration profile applies three current Flyway migrations; the fully enabled Identity runtime is `UP`/Ready simultaneously with the other four services in the local integrated runtime using explicit positive local/CI-fixture capacity values while fail-closed zero production defaults remain unchanged; protected CI remains commit-specific; deployed staging/production runtime remains NOT VERIFIED | NOT VERIFIED | `services/identity-service` |
| Authorization Service | DESIGNED | IMPLEMENTED current repository slice | canonical WSL Java 25 strict Gradle + PostgreSQL/Redis Testcontainers gates pass; the real migration profile applies the current Flyway migration; Authorization is runtime-enabled and `UP`/Ready simultaneously with the other four services against its non-owner local DB role and local Redis/key/JWT-verifier fixtures; protected CI remains commit-specific; deployed staging/production runtime remains NOT VERIFIED | NOT VERIFIED | `services/authorization-service` |
| Notification Service | DESIGNED | IMPLEMENTED | canonical WSL Java 25 strict Gradle + PostgreSQL Testcontainers gates pass; the real migration profile applies both current Flyway migrations; Notification is `UP`/Ready simultaneously with the other four services using its local-only simulated providers and non-owner local DB role; repository CI evidence remains commit-specific; production Liara/IPPanel provider/runtime evidence NOT VERIFIED | NOT VERIFIED | `services/notification-service` |
| Web BFF | DESIGNED | IMPLEMENTED current repository slice | canonical WSL Java 25 strict Gradle + Redis Testcontainers gates pass; repository `application.yaml` route-audience binding and bounded 404 mapping are regression-tested; the runtime-enabled BFF is `UP`/Ready over local self-signed HTTPS at `https://localhost:18443` while all four internal services are simultaneously running; browser-session bootstrap returns `201 / PREAUTH`, and a valid bootstrapped session reaches Identity over gRPC where an unknown local-login identity returns the expected non-enumerating `401 / AUTHENTICATION_FAILED`; protected CI remains commit-specific; deployed edge/WAF runtime remains NOT VERIFIED | NOT VERIFIED | `services/web-bff` |
| Compromised Password Service | DESIGNED | IMPLEMENTED | canonical WSL Java 25 strict Gradle/integration/bootJar gates pass; the repository-built `GENERATED_TEST_FIXTURE` dataset runtime is `UP`/Ready simultaneously with the other four services in the local integrated runtime; repository CI evidence remains commit-specific; production HIBP corpus/runtime evidence NOT VERIFIED | NOT VERIFIED | `services/compromised-password-service` |
...
| ChatGPT Web developer-host Ops MCP | DESIGNED under ADR-0048/0051 | IMPLEMENTED external Windows runtime + host integration | independent runtime Context 14/14 and Ops 21/21 suites pass; live local command ceiling is 300s and Context/Ops wrappers request 1h MCP connection TTL; host audit proves >180s local execution without local timeout, while synchronous tunnel response above the shorter measured response lifetime remains NOT VERIFIED and must use split/checkpoint or persistent-terminal/polling behavior; production authority NOT APPLICABLE |
