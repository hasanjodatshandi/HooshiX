...
ADR-0048 adds a separate developer-host Ops MCP. Repository implementation includes a mandatory local-policy parser, typed bounded filesystem operations, policy-alias process execution, sanitized child environment, bounded output/time/audit behavior, explicit UTF-8 stdio framing, and deterministic tests. Operator execution on 2026-08-18 verified protected local Ops policy/key ACLs, intended elevated Windows token, separate tunnel/profile/runtime key, health/readiness, ChatGPT `ops.status`, bounded filesystem/process smoke, metadata audit, Scheduled Task highest-privilege background operation, and reboot recovery on the operator PC. Ops MCP remains developer-host only and does not change ADR-0030.

ADR-0049 adds a third developer-host Desktop MCP. Repository implementation includes strict local Desktop policy/version parsing, a pinned WinApp CLI adapter, fresh HWND/process app authorization, bounded UI inspection and transient PNG capture, UIA/mouse/keyboard/system-key controls, sanitized WinApp child environment with telemetry opt-out, metadata-only fail-closed audit, screenshot MCP image output, explicit UTF-8 stdio framing, and deterministic tests. Operator execution on 2026-08-18 installed and verified Microsoft WinApp CLI `0.6.0`, then exercised the repository Desktop engine in a temporary **non-elevated interactive** Scheduled Task against a disposable Notepad file: fresh HWND/UIA discovery, focus, real mouse click/hover, lowercase literal keyboard typing, `Ctrl+S`, bounded PNG screenshot, persisted-file verification, audit redaction, and capture-temp cleanup passed. A separate mixed-case probe delivered all characters but changed the case of one uppercase character; this is retained as a WinApp/keyboard-fidelity limitation rather than reported as exact-text success. Protected final Desktop policy/ACLs, separate Desktop tunnel/profile/key, ChatGPT discovery/calls, persistent logon task, and revocation/rollback remain **NOT VERIFIED**. Desktop MCP is not a credential/UAC/Secure-Desktop or production administration path.
...
- ADR-0049 Desktop MCP source/policy schema/tests/runbook are present; WinApp 0.6.0 and a temporary non-elevated interactive Notepad UIA/mouse/keyboard/screenshot/audit smoke passed, with one mixed-case fidelity limitation observed; separate Desktop tunnel/ChatGPT/persistent-task/revocation evidence remains NOT VERIFIED;
...
| ChatGPT Web developer-host Desktop MCP | DESIGNED under ADR-0049 | IMPLEMENTED repository integration + partial host smoke | repository tests present; WinApp 0.6.0 and temporary non-elevated interactive Notepad UIA/mouse/lowercase-keyboard/shortcut/screenshot/audit smoke passed; mixed-case fidelity edge observed; separate tunnel/ChatGPT/persistent-logon/revocation evidence NOT VERIFIED |
...
- real Windows/ChatGPT Web ADR-0049 Desktop evidence for protected policy/audit/capture ACLs, intended non-elevated interactive session, separate tunnel credential/profile, `desktop.status`, bounded screenshot/inspect/UIA/mouse/keyboard calls, audit behavior, and logon restart/rollback;
