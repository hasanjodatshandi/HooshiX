...
Authorization and Web BFF application services are implemented as current repository slices. The coherent change that introduces BFF OpenAPI 3.1 version 1.2.0 establishes coverage for all 36 implemented public controller method/path mappings, with schema validation, consumer examples, controller/OpenAPI parity, and generated frontend transport-type drift enforcement; its executed checks remain change-specific PR/CI evidence and are not inferred by this status summary. Web BFF also provides trusted-client-address forwarding, RFC 9457 public errors, and the authoritative-security Identity registration edge. Identity has the registration plus local-password authentication/Session/RefreshFamily/JWT-signing and Tenant/Membership/Invitation/selection slices described above. Notification includes its delivery runtime and Liara/IPPanel adapter implementation, while real production provider execution remains NOT VERIFIED. The local kind production-fidelity lane provides deployed integration evidence for the current five services and local observability stack. No production platform runtime, production provider/corpus approval, restore exercise, complete-stack load test, executed production artifact-signing release, executed production admission chain, or production traffic readiness is claimed. The repository release/admission automation is implementation evidence only until a reviewed production release and environment run it.

## Capability/service status

| Capability | Architecture | Independent implementation | Runtime evidence | Production readiness | Planned target |
| --- | --- | --- | --- | --- | --- |
| Identity Service | DESIGNED | IMPLEMENTED registration + local-password authentication/Session/RefreshFamily/JWT + password/Profile/Contact lifecycle + Tenant/Membership/Invitation/selection repository slices | current local revision: canonical WSL Java 25 strict Gradle + PostgreSQL/Redis Testcontainers, non-owner forced-RLS/pool-reuse negatives, nine Flyway migrations including V5 tenant/session Membership identity constraints, V6 Notification result correlation, V7/V8 password recovery/outbox/session-revocation constraints, and V9 secure Profile/Contact challenge/idempotency constraints; password/Profile/Contact lifecycle unit/persistence/transport tests, immutable key-ID refresh checks, bounded global zero-queue gRPC admission, Tenant/password/Profile lifecycle metrics, Notification terminal-result callback persistence, Buf compatibility, Semgrep, OSV, Gitleaks, Helm/render, Prometheus/dashboard, and digest-pinned runtime-image checks pass locally; protected CI remains PR-head-specific; deployed password/Profile runtime and production runtime remain NOT VERIFIED | NOT VERIFIED | `services/identity-service` |
...
| Web BFF | DESIGNED | IMPLEMENTED current repository slice including Public Registration, Profile/Contact, Password Lifecycle, full public REST contract coverage + OpenAPI | OpenAPI 3.1 version 1.2.0 covers all 36 implemented public controller method/path mappings with schema validation, consumer examples, controller/OpenAPI parity, and generated frontend transport-type drift enforcement. This is repository implementation evidence only once the coherent change merges; its test execution remains PR/CI-specific. RFC 9457 problems, anonymous same-origin registration/recovery security, session-bound CSRF, trusted client-address forwarding, server-only refresh credential custody, and atomic browser security-state rotation remain required. Deployed password journey and production edge/runtime remain NOT VERIFIED | NOT VERIFIED | `services/web-bff` |
