...
Provider safety/evaluation/telemetry and cancellation propagation, BFF/UI route,
...
| Conversation Service | DESIGNED under ADR-0054/0057 | PARTIAL: executable foundation, authority boundary, private Conversation CRUD/lifecycle, encrypted Message history, budgeted ModelRun execution/cost reconciliation, tenant lifecycle, and ADR-0028 erasure repository slices IMPLEMENTED; product vertical slice pending | Local unit/integration evidence covers exact-audience JWT, one-attempt 300ms Authorization, validated/bounded gRPC, Membership ownership, AES-GCM content, exact governed model/prompt/price policy, atomic reservation, durable replay/cancellation, metadata-only lease queue, tenant-bounded context and concurrency, one-attempt stateless OpenAI Responses adapter, conservative ambiguous/expired outcome handling, integer reconciliation, ordered fail-closed lifecycle events, idempotent erasure Inbox, service-owned purge, and durable non-PII receipt Outbox. PR #134 protected baseline `35466974425` and frontend E2E `35466974204`, plus post-merge baseline `35467376618` and frontend E2E `35467376506`, passed at exact merge `2f450003fa50f0e83ce3321799d922e2467e4d92`; protected evidence for the active lifecycle/erasure slice remains pending. Governance and cluster egress keep provider execution disabled; provider safety/evaluation/telemetry and transport cancellation, BFF/frontend, activation/release, and deployed runtime remain NOT VERIFIED | NOT VERIFIED | `services/conversation-service`; `mlops/` |
