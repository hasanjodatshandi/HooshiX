...
| Conversation Service | DESIGNED under ADR-0054/0057 | PARTIAL: executable security/data/deployment foundation and authority boundary IMPLEMENTED; product vertical slice pending | Local strict Gradle unit/integration/architecture/coverage/SpotBugs/JAR checks, PostgreSQL 18.4 Flyway/role/forced-RLS/pool-reuse negatives, AES-256-GCM binding/tamper/key-rotation tests, exact-audience RSA JWT validation/key-staleness tests, single-attempt 300ms Authorization deny/outage/concurrency tests, dependency registry, least-privilege Helm/network/mesh render, Prometheus rules, and dashboard JSON pass for this slice. Provider execution remains disabled and API/resource-ownership/lifecycle/erasure/worker/cost/BFF/frontend/release/deployed-runtime evidence is NOT VERIFIED | NOT VERIFIED | `services/conversation-service`; `mlops/` |
