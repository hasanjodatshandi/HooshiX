...
This is an architecture decision, not production execution evidence. Current repository implementation status is: service-specific Semgrep and OSV locked-dependency scanning exist for the implemented service slices; all five implemented Java service workflows also implement Gitleaks current-tree/Git-history scanning; Syft/Grype/Cosign exact-digest release automation, retained release/rescan evidence handling, and stable Kyverno production release-admission generation are implemented in the repository. Real production registry scanning/signing/attestation, deployed-digest rescanning, and production-cluster Kyverno enforcement remain `NOT VERIFIED`; see `implementation-status.md` for the current evidence boundary.
...
The repository contains repository-governance CI plus executable Compromised Password, Notification, Identity registration/authentication/Session/RefreshFamily/JWT/Tenant/Profile/Contact/MFA/ExternalIdentity/erasure slices, Authorization, and Web BFF under their service roots. Service-specific Semgrep and OSV locked-dependency gates are present, and all five implemented Java services include Gitleaks tree/history gates. The current ADR-0012 repository surface is implemented; the ADR-0054 Conversation/ModelRun product slice and broader hardening evidence remain incomplete. In addition to the fast WSL application runtime under `infrastructure/local/`, root `deploy/` and repository-owned local production-fidelity infrastructure now implement and locally verify kind/Calico/Gateway API, Istio Ambient, Kyverno CEL admission, Traefik/WAF, local staging PostgreSQL/Redis, the five current services, and the approved observability component family. This local kind/staging evidence does not prove the selected production K3s/CNPG/OpenBao/Kafka/GitOps/host-access environment, production corpus/provider evidence, real production execution of the ADR-0045 final-artifact release/admission chain, capacity/DR, or production readiness; those remain recorded separately in `implementation-status.md`.
