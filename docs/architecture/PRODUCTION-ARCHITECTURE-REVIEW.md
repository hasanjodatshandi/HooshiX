...
This is an architecture decision, not production execution evidence. Current repository implementation status is: service-specific Semgrep and OSV locked-dependency scanning exist for the implemented service slices; all six Java service workflows also implement Gitleaks current-tree/Git-history scanning; Syft/Grype/Cosign exact-digest release automation, retained release/rescan evidence handling, and stable Kyverno production release-admission generation remain implemented for the existing release boundary. Conversation is only a Stage 9 foundation and is not yet production-release eligible. Real production registry scanning/signing/attestation, deployed-digest rescanning, and production-cluster Kyverno enforcement remain `NOT VERIFIED`; see `implementation-status.md` for the current evidence boundary.
...
The repository contains repository-governance CI plus executable Compromised Password, Notification, Identity registration/authentication/Session/RefreshFamily/JWT/Tenant/Profile/Contact/MFA/ExternalIdentity/erasure slices, Authorization, Web BFF, and the initial Conversation security/data/deployment foundation under their service roots. Service-specific Semgrep, OSV, and Gitleaks gates are present for all six Java services. The ADR-0054 Conversation/ModelRun product slice remains incomplete beyond that foundation. The fast WSL and production-fidelity lanes still integrate the five product-ready pre-Conversation services; Conversation staging/release integration is intentionally pending its Stage 9 API, Authorization, lifecycle/erasure, worker/provider/cost, and activation gates. Existing local kind/staging evidence does not prove the selected production K3s/CNPG/OpenBao/Kafka/GitOps/host-access environment, production corpus/provider evidence, production release/admission execution, capacity/DR, or production readiness; those remain recorded separately in `implementation-status.md`.
