...
- final-image Syft SBOM generation is **IMPLEMENTED IN STAGE 10 SOURCE / PROTECTED CI AND PRODUCTION EXECUTION NOT VERIFIED** for all seven application release components, including `conversation-service` and `web-frontend`, in the protected production release workflow and is retained as digest-bound CycloneDX evidence;
