...
| SEC-035 | Developer-host Desktop MCP cannot silently expand Context/Ops authority, target a denied/unresolved process, expose arbitrary WinApp/PowerShell/credential/Secure-Desktop control, retain capture/typed data in audit/argv/environment, or inject mouse/keyboard/system keys without explicit local policy | exact separate tool-surface test + strict policy/version/session/app/HWND negatives + semantic-selector/capability/key-grammar bounds + fixed isolated Unicode-helper stdin/argv/environment/timeout/error tests + PNG/temp cleanup + audit fail-closed/redaction/rotation + UTF-8/MCP-image tests + operator exact-Unicode/WinApp/tunnel/interactive smoke/restart evidence |
...
ADR-0049 permits a third developer-host Desktop MCP only through explicit local app/capability/session policy and fresh HWND/process authorization. It does not extend Context or Ops, cannot bypass UAC/Secure Desktop/SAS, and has no credential-reader/arbitrary-command surface. Visible UI/screenshot content can still contain sensitive data and therefore requires operator-authorized use plus non-content audit metadata.
