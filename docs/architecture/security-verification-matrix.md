...
| SEC-034 | Developer-host Ops MCP cannot silently expand Context authority, escape configured path roots, run unapproved/unbounded commands, inherit tunnel/API credentials into child processes, or gain elevated mutation/execution without explicit local policy | independent Windows runtime tool-surface test + denied-root/symlink/path negatives + command-alias/argv/cwd/timeout/output-bound tests + child-environment secret exclusion + elevation opt-in negatives + audit/rotation tests + explicit UTF-8 stdio regression + operator host/tunnel evidence |
