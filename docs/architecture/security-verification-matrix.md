...
| SEC-033 | Agent context cannot silently outrank current Git, expose mutation authority, accept stale/ambiguous narrow scope, forge post-merge continuity, or turn ChatGPT Web access into public/general host authority | clean/dirty bootstrap provenance + route escalation + tracked-file bounds/sensitive-name exclusion + work/post-merge provenance negatives + read-only stdio tool-list/unknown-write negatives + independent Context adapter tests + fixed WSL policy/root negatives + ADR-0047/0051 tunnel review proving stdio child/no HooshiX listener/restricted runtime credential/no shell or arbitrary filesystem tool |
| SEC-034 | Developer-host Ops MCP cannot silently expand Context authority, escape configured filesystem/job-state roots, run unapproved/unbounded commands/jobs, inherit tunnel/API credentials into child processes, turn cancellation into arbitrary PID termination, or gain elevated mutation/execution without explicit local policy | independent Windows runtime exact tool-surface test + denied-root/symlink/reparse/job-ID/path negatives + command-alias/argv/cwd/timeout/output/job-capacity/retention/log-page bounds + detached-runner restart/timeout/cancel tests + transient-argv/persistent-metadata redaction + child-environment secret exclusion + elevation opt-in negatives + audit/rotation tests + explicit UTF-8 stdio regression + operator host/tunnel evidence |
| SEC-035 | Developer-host Desktop MCP cannot silently expand Context/Ops authority, target a denied/unresolved process, expose arbitrary WinApp/PowerShell/credential-reading/Secure-Desktop control, retain capture/typed/credential data in audit/argv/environment, or use a local credential outside an explicit app/executable-path/SHA-256/password-target binding | independent Windows runtime exact tool-surface test + strict policy/version/session/app/HWND negatives + semantic-selector/capability/key-grammar bounds + fixed Unicode-helper and credential-helper stdin/argv/environment/timeout/error tests + broker schema has no secret input/output + Generic Credential/focused-IsPassword/unique-password/PID/same-focus binding tests + PNG/temp cleanup + audit fail-closed/redaction/rotation + UTF-8/MCP-image tests + operator disposable-credential/WinApp/tunnel/interactive smoke/restart evidence under ADR-0049/0050/0051 |
| SEC-036 | Conversation/model execution cannot cross tenant/private-content boundaries, convert model output into authority/side effects, leak content/credentials, or create unbounded/free ambiguous provider work | exact JWT audience + online Authorization + owner/RLS negatives; encrypted-content/key tamper; same-Conversation context proof; fixed-egress `store=false`/no-tool provider fixture; sanitized render; telemetry/event/audit canaries; atomic budget/overflow/concurrency/cancel/unknown-outcome/erasure tests under ADR-0054 |
...
ADR-0048 permits a separate developer-host Ops MCP only through an explicit local policy and host process token. It does not extend Context MCP authority and does not replace ADR-0030 production JIT controls. If the operator explicitly permits an elevated general interpreter, that capability is broad local-host administration and must be treated as such; policy/path checks on typed filesystem tools are not claimed to sandbox that interpreter.

ADR-0049 permits a third developer-host Desktop MCP only through explicit local app/capability/session policy and fresh HWND/process authorization. ADR-0050 permits one additional optional use-without-disclosure primitive: `desktop.use_credential` accepts only an opaque local reference and can use one pre-provisioned Generic Credential only at its policy-bound app/executable-path/SHA-256/password target. It has no credential read/list/write/export surface, does not make `desktop.type_text` secret-capable, does not extend Context or Ops, and cannot bypass UAC/Secure Desktop/SAS. Visible UI/screenshot content can still contain sensitive data and therefore requires operator-authorized use plus non-content audit metadata.
