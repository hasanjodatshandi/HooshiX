...
| SEC-017 | Production human access: zero standing privilege, phishing-resistant authentication, approvals, short TTL, protected audit; single-server hardened OpenSSH FIDO2 + real system/privilege audit, HA Teleport | profile-specific access-plane policy + positive/negative auth + JIT expiry + session/audit evidence |
...
- hardened OpenSSH/FIDO2 must use real durable audit, not `.bashrc` or shell history;
