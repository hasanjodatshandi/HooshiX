...
| SEC-008 | Service communication: dedicated workload identity, strict mTLS, least-privilege Istio authorization, deny-by-default NetworkPolicy and BFF exact egress allow-list | positive/negative deployment policy + wrong-workload/arbitrary-egress tests |
...
| SEC-013 | Browser hardening: same-origin-only v1, mandatory Fetch Metadata on unsafe production browser requests, exact CSP without unsafe-inline/eval, HSTS/nosniff/referrer/Permissions-Policy, no-store for auth/session/admin, safe redirects, no unsafe raw HTML/private service-worker caching | header/browser/source/OpenAPI tests |
...
| SEC-015 | Runtime/container hardening: non-root, no privilege escalation, dropped capabilities, RuntimeDefault seccomp, read-only FS where possible, BFF >=3/PDB2/HPA gating | rendered manifest/policy tests |
