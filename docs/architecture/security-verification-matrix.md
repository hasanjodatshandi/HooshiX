...
| SEC-035 | Developer-host Desktop MCP cannot silently expand Context/Ops authority, target a denied/unresolved process, expose arbitrary WinApp/credential/Secure-Desktop control, retain capture/typed data in audit, or inject mouse/keyboard/system keys without explicit local policy | exact separate tool-surface test + strict policy/version/session/app/HWND negatives + semantic-selector/capability/key-grammar bounds + PNG/temp cleanup + child-environment secret exclusion + audit fail-closed/redaction/rotation + UTF-8/MCP-image tests + operator WinApp/tunnel/interactive smoke/restart evidence |
