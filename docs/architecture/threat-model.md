...
- ADR-0049 Desktop MCP is a third developer-host stdio surface with mandatory local policy, pinned WinApp version, intended interactive/non-elevated session, fresh HWND/process app authorization, explicit screenshot/UIA/mouse/keyboard/system-key flags, bounded transient capture, metadata-only audit, separate tunnel/profile/key, and no credential/UAC/production authority;
...
| Information disclosure | Desktop capture/UI text/typed input is retained in local audit or used as a credential-reader path | bounded transient PNG cleanup + no clipboard/get-value/credential tool + audit stores only digests/metadata and never raw title/selector/text/screenshot/output | ADR-0049 screenshot/audit redaction/fail-closed tests + host evidence |
...
- developer-host Desktop threats -> ADR-0049 policy/version/session/app/HWND/capture/UIA/mouse/keyboard/environment/audit/UTF-8 tests + separate operator-PC WinApp/tunnel/interactive/background evidence;
