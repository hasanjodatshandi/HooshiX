...
| `conversation.authorization-permission-check` | Conversation | Authorization `CheckPermission` | `AUTHORITATIVE_SECURITY` | Conversation operation unavailable; fail closed | none | none; no permission cache/stale allow | Conversation | ADR-0013, ADR-0026, ADR-0054; `services/conversation-service.md` |
| `conversation.model-provider-execution` | Conversation | OpenAI Responses | `EXTERNAL_SIDE_EFFECT` | preserve definitive rejection/unavailability or `OUTCOME_UNKNOWN`; reconcile reserved cost | none | none; no retry, alternate model, or alternate provider | Conversation | ADR-0054, ADR-0057; `services/conversation-service.md`; `mlops-evaluation-and-safety.md` |
...
| `identity.password-credential-write` | Identity | Compromised Password `LookupPrefix` | `AUTHORITATIVE_SECURITY` | reject unchecked password; dataset/query failure never becomes clean result | none | none; no external-provider/Redis/PostgreSQL fallback | Identity | ADR-0040; `security-architecture.md` §4; `services/identity-service.md` §3; `services/compromised-password-service.md` §2 |
...
| `identity.authorization-tenant-permission-check` | Identity | Authorization `CheckPermission` | `AUTHORITATIVE_SECURITY` | tenant lifecycle administration unavailable; fail closed | none | none; no permission cache/stale allow | Identity | ADR-0012, ADR-0013, ADR-0026; `services/identity-service.md`; `services/authorization-service.md` |
...
| `identity.erasure-command-publication` | Identity | Kafka `hooshix.identity.erasure.command.v1` | `DURABLE_COMMAND` | command outbox remains pending or explicitly exhausted; broker acknowledgement never completes the request | Identity durable erasure command dispatcher | none; PostgreSQL coordination state/outbox remains authority | Identity | ADR-0015, ADR-0028; `services/identity-service.md` |
| `erasure.participant-receipt-publication` | required erasure participant | Kafka `hooshix.identity.erasure.receipt.v1` | `DURABLE_COMMAND` | receipt outbox remains pending or explicitly exhausted; Identity keeps the request incomplete | participant durable receipt dispatcher | none; participant database effect and receipt outbox remain authority | participating bounded context | ADR-0015, ADR-0028; `data-and-messaging.md` |
| `notification.provider-dispatch` | Notification | email/SMS provider | `EXTERNAL_SIDE_EFFECT` | preserve ambiguity and reconcile | Notification worker/lifecycle | never fabricate ACCEPTED/DELIVERED | Notification | ADR-0006, ADR-0007, ADR-0055, ADR-0056 |
...
| `web-bff.identity-local-password-authentication` | Web BFF | Identity `AuthenticateLocal` | `AUTHORITATIVE_SECURITY` | login unavailable; fail closed; no alternate credential acceptance | none | none | Web BFF | ADR-0012, ADR-0016; `services/identity-service.md` §4; `services/web-bff.md` §4 |
| `web-bff.identity-mfa` | Web BFF | Identity `IdentityMfaService` | `AUTHORITATIVE_SECURITY` | MFA login or lifecycle operation unavailable; fail closed; no Session completion or state-change claim | none | none; no local proof, stale challenge, factor downgrade, or browser credential authority | Web BFF | ADR-0012, ADR-0016, ADR-0024; `services/identity-service.md`; `services/web-bff.md` |
| `web-bff.identity-password-lifecycle` | Web BFF | Identity `IdentityPasswordService` | `AUTHORITATIVE_SECURITY` | password change/recovery unavailable or rejected according to Identity result; fail closed | none | none; no browser credential authority or local password decision | Web BFF | ADR-0012, ADR-0016, ADR-0024, ADR-0053; `services/identity-service.md`; `services/web-bff.md` |
| `web-bff.identity-registration` | Web BFF | Identity `IdentityRegistrationService` | `AUTHORITATIVE_SECURITY` | registration unavailable or rejected according to Identity result; fail closed | none | none | Web BFF | ADR-0009, ADR-0016, ADR-0024; `services/identity-service.md` §§2,6,8; `services/web-bff.md` §12 |
| `web-bff.identity-session-refresh` | Web BFF | Identity `RefreshSession` | `AUTHORITATIVE_SECURITY` | session refresh unavailable; fail closed; no browser reconstruction | none | none | Web BFF | ADR-0012, ADR-0016; `services/identity-service.md` §4; `services/web-bff.md` §4 |
| `web-bff.identity-session-revocation` | Web BFF | Identity logout current/all | `AUTHORITATIVE_SECURITY` | Identity revocation outcome unavailable; BFF may discard its browser session but must not claim global family revocation | none | none | Web BFF | ADR-0012, ADR-0016; `services/identity-service.md` §4; `services/web-bff.md` §4 |
...
| `web-bff.conversation-api-dispatch` | Web BFF | Conversation `ConversationService` | `AUTHORITATIVE_STATE` | abort request; private Conversation and ModelRun state unavailable; no fabricated result | none | none | Web BFF | ADR-0054; `services/conversation-service.md`; `services/web-bff.md` |
...
| `service.required-security-audit-evidence` | owning service | audit persistence/outbox | `AUTHORITATIVE_STATE` | durable contract; do not silently drop | owning service | none unless explicit current decision | owning service | ADR-0033; `security-architecture.md` §12 |
...
- Erasure Kafka publication is a durable-command edge in both directions: local Outbox/Inbox and participant effects remain authoritative, and transport failure may delay but never fabricate completion.
...
- BFF password lifecycle transport is authoritative security; only Identity decides credential proof/policy/recovery, and the browser never supplies or receives a refresh credential.
