...
| AFF-054 | Inter-service transport contracts are consumed only through the versioned contract package and not service source trees | repository scan for service-local proto ownership, cross-service paths, contract artifact build, Buf lint/breaking checks | PR | block coupling regression |
