...
| AFF-051 | Agent context is Git-provenanced, conservative under uncertainty, and reachable only through approved bounded read-only interfaces | HooshiX bootstrap/router/checkpoint/search tests + generated matrix check + independent Windows Context adapter/WSL-policy tests + ADR-0047/0051 tunnel review proving no HooshiX network listener/write/general-shell expansion | PR/host | block misleading/narrow/overprivileged agent context |
| AFF-052 | Developer-host Ops MCP remains separate from Context authority, policy-gated, path/command/job-state bounded, auditable, explicit-UTF-8 stdio, and outside production administration | independent Windows Ops runtime policy/path/process/persistent-job/environment/elevation/audit/UTF-8 tests + operator host/tunnel evidence under ADR-0048/0051 | PR/host | block overprivileged, unbounded, or misleading Ops capability |
| AFF-053 | Developer-host Desktop MCP remains separate from Context/Ops, uses reviewed WinApp/session/app/HWND/capability bounds plus fixed isolated text/credential helpers, keeps sensitive capture/input/credential values out of MCP/Python/audit/argv/environment, and can use a credential only through an explicit local app/executable-path/SHA-256/password-target binding without becoming a credential-reader/UAC/production bypass | independent Windows Desktop runtime policy/version/app/HWND/capture/input/helper/credential/audit/MCP tests + operator WinApp/tunnel/interactive/restart evidence under ADR-0049/0050/0051 | PR/host | block overbroad, privacy-unsafe, wrong-target, or misleading Desktop capability |
| AFF-054 | Inter-service transport contracts are consumed only through one SemVer contract artifact, every RPC request carries executable schema validation, every published service contract has a tested consumer example, and every serving workload installs fail-closed validation | repository scan for service-local proto ownership/cross-service paths, exact producer/consumer artifact version, versioned proto/Java packages, Protovalidate annotations/runtime interceptor, executable JSON fixtures, Buf lint/build/breaking checks | PR | block coupling, version, validation, or documentation drift |
| AFF-055 | Every implemented public Web BFF controller method/path mapping is represented by the SemVer-versioned OpenAPI contract with schema validation and a consumer example, and generated frontend transport types are current | controller/OpenAPI mapping parity, contract version/operation/schema/example checks, compatibility review, generated-client freshness/drift check | PR | block public REST contract or client drift |
| AFF-056 | The ADR-0054 first slice remains one private Conversation boundary with stateless no-tool model execution, authoritative resource/cost state, bounded work, and erasure/lifecycle safety | service-boundary/ArchUnit scan + contract validation/examples + RLS/owner/key/privacy/provider-option/fixed-egress/budget/ambiguity/cancel/erasure/load/browser tests | PR/release | block speculative boundary, authority, privacy, financial, or reliability drift |
...
- ADR-0048 adds a separate developer-host Ops MCP. Its local policy, OS token, bounded roots/command aliases/limits, persistent-job count/state/output bounds, runner-owned timeout/cancellation, explicit elevation opt-ins, and metadata audit constrain the exposed Ops surface. Persistent jobs decouple long local work from one tunnel response but do not increase the local timeout or create a synchronous transport SLA. Allowing an elevated general interpreter is explicit broad local-host authority, not a sandbox and not production JIT authority.
- ADR-0049 adds a third developer-host Desktop MCP. ADR-0050 adds an opt-in policy-bound credential-use broker to that same boundary. The broker accepts no secret value, fixes app/executable-path/SHA-256/selector/Credential Manager target in local policy, verifies fresh app authorization plus exact process-image path/SHA-256 and focused UIA password semantics before local `CredReadW`, keeps the credential out of MCP/Python/output/audit/argv/environment, and does not create a credential reader, UAC/Secure-Desktop path, or production authority.
