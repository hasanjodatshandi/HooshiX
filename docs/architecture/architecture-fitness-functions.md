...
| AFF-032 | Public client-address authority and origin restriction | external-L4-only Traefik-origin firewall/routing denial for direct Internet/non-approved sources + external-L4 PROXY-v2 source preservation + exact Traefik trusted CIDRs + insecure-mode denial + Caddy strict trusted-proxy parsing/internal-header overwrite + BFF one-IP parsing + forged-header/untrusted-PROXY/proxy-address/IPv4/IPv6/mapped-address negatives + raw-IP leak negatives | PR/release | block public quota-protected traffic |
...
- ADR-0043 defines client-address and management-network trust. Network/proxy failure is not permission to use caller forwarding headers, allow direct Internet-to-Traefik-origin access, or enable public SSH.
