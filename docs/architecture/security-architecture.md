...
ADR-0041 Reference Data contains only global public standard reference metadata and no User/Tenant/Membership/session state. It is not an ADR-0028 erasure participant in v1. Adding subject-linked or tenant-specific state requires a new ownership/security/erasure review before implementation.
...
Web BFF is the only browser-facing application API boundary. v1 public REST lives under `/api/v1`, with `/api/v1/auth`, `/api/v1/identity`, `/api/v1/authorization`, and `/api/v1/reference` as reviewed subspaces. Internal RPC names are not mechanically exposed as public routes.

ADR-0041 `/api/v1/reference` is an explicit read-only GET/HEAD facade for global public Country/Currency/TimeZone/SupportedLocale data. It may be anonymous and therefore does not require an authenticated BFF session or CSRF proof merely to read the reference representation. This exception creates no user/tenant/JWT authority, does not enable cross-origin credentialed CORS, and does not bypass the mandatory upstream mitigation -> Traefik -> WAF -> BFF path. Representation locale is explicit `fa|en` rather than hidden session/cookie state.

Public input is bounded before expensive work: JSON <=256KiB, auth/OIDC/session body <=64KiB, request headers/metadata <=16KiB, and multipart/file upload is outside v1. Reference Data GET/HEAD has no request body. Public RFC 9457 errors expose only `type`, `title`, `status`, stable `code`, and an optional safe correlation identifier; tenant/membership/Contact IDs, internal request IDs, provider/source/exception text, tokens, Role/permission internals, source-artifact paths, and security details remain private.
...
After all required factors, a User with no selected active Membership may have only `authenticated_onboarding`: no normal tenant-scoped resource JWT and only reviewed Identity onboarding/profile/tenant-create/invitation-accept/tenant-selection allow-list. Zero Membership remains onboarding; one valid Membership selects automatically; multiple follow Identity last-valid/explicit selection. Tenant selection rotates BFF session identity. Public Reference Data GET/HEAD remains globally readable during onboarding without expanding that allow-list into resource authority.
...
Unsafe cookie-authenticated production browser requests require all of trusted exact `Origin`, valid `X-CSRF-Token`, and `Sec-Fetch-Site: same-origin`. Missing/invalid Fetch Metadata on normal browser production routes fails closed. GET/HEAD/OPTIONS remain side-effect free. Public Reference Data v1 uses safe GET/HEAD only and does not require CSRF proof. A future non-browser client that cannot satisfy this model requires a separately reviewed surface.

Cross-origin credentialed CORS is disabled in v1: same-origin only. Anonymous Reference Data does not create a CORS exception. Future CORS requires architecture review; wildcard/reflected credentialed origins remain prohibited.
...
`unsafe-eval` and `unsafe-inline` are prohibited. Additional external sources/directives require explicit review. Authentication/OIDC/session/Authorization-management responses use `Cache-Control: no-store`. ADR-0041 Reference Data is the explicit cacheable exception: successful public representations use deterministic `ETag` plus `Cache-Control: public, max-age=3600`; BFF has no server-side stale Reference Data fallback. HSTS after domain-coverage verification, nosniff, strict-origin-when-cross-origin referrer policy, restrictive Permissions Policy, and CSP frame protection are centrally tested.
...
Dependency ownership follows the machine-readable registry: Web BFF owns provider-protocol edge to Google, trusted evidence/session edge to Identity, authoritative Identity audience-token brokerage, session Redis, semantic-quota Redis, Authorization tenant management, Reference Data read, and registered resource dispatch. Identity does not own direct Google login/link dependency. Identity owns explicit edges to semantic-quota Redis, compromised-password service, Notification durable handoff, Authorization owner/member provisioning, owner-safe Membership-removal prepare/resolution, tenant lifecycle synchronization, and authoritative `CheckPlatformPermission` for platform tenant/legal-hold operations.
...
ADR-0041 Reference Data, when implemented, is ClusterIP-only and initially accepts application gRPC only from the `platform-apps/web-bff` ServiceAccount. A new Reference Data caller requires a new dependency-registry entry and explicit NetworkPolicy/Istio authorization. The Reference Data service itself has no standards-source/provider Internet synchronization; application egress is deny-by-default with only narrowly required DNS/approved telemetry paths.

Web BFF NetworkPolicy/Istio egress is deny-by-default and allows only Identity, Authorization management, Reference Data typed read when active, explicitly registered resource services, BFF/security Redis, configured Google OIDC endpoints, and approved telemetry backend/collector. Arbitrary URL/Internet egress is prohibited. Every new synchronous downstream requires dependency-registry entry before production.
...
Reference Data needs no runtime standards-source credential in v1. Source provenance, integrity and license/use-right evidence belong to the offline release/import process and must not be transformed into a runtime Internet credential path.
...
Anonymous `/api/v1/reference` remains on this same public edge path and does not create a direct Reference Data ingress route.
...
ADR-0041 Reference Data source-revision manifest/content digest is part of release provenance. Offline source acquisition validates provenance/integrity/license use before bundle publication; production does not fetch standards data merely because the signed application image is vulnerable/unavailable.
...
ADR-0041 Reference Data target, only after its implementation trigger, uses `platform-apps/reference-data-service`, gRPC 9090, separate management port, minimum 3 replicas, PDB `minAvailable=2`, topology spread and HPA only after load evidence. Readiness requires a compatible validated local immutable bundle/security configuration; liveness is local process progress and does not probe Internet source authorities.

## 13. Logging/PII and privileged access

Logging is structured and allow-list based. Raw passwords/OTP/recovery codes/tokens/cookies/keys/secrets/payment data/high-risk identity data/full sensitive payloads/SQL binds/complete gRPC metadata/Kafka headers/unreviewed provider/source payloads are prohibited.

Ordinary PII requires approved purpose and masking/tokenization or managed-key HMAC pseudonymization where correlation is needed. Input-derived fields are CR/LF-safe; exception/provider/source text is untrusted until sanitized. Metric labels remain low-cardinality and exclude business/security IDs, trace IDs, raw URLs, page tokens, arbitrary reference values, and free-form errors.
...
Reference Data routine telemetry may include bounded operation/family, outcome, latency, bundle readiness/version from the deployed set and safe aggregate record counts. It does not log full imported source artifacts/responses, source paths/URLs, page tokens or unbounded source metadata.
