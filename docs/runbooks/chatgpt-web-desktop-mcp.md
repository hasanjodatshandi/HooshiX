...
Use the real normalized process name from `desktop.list_windows`. Resolve the exact executable path for that PID and calculate SHA-256 from the executable file, then store both in the protected local binding. A binary update changes the hash and must fail closed until the binding is reviewed and updated. Prefer a stable semantic selector confirmed through `desktop.inspect`. If the password control has no stable AutomationId, use the reserved `@unique-password` strategy only after host verification. `@unique-password` is UIA-first: multiple enabled/visible UIA password matches fail closed; exactly one is used directly. Only when UIA reports zero password matches may the helper accept exactly one same-PID enabled/visible single-line native `Edit`/`WindowsForms10.EDIT.*` child with `ES_PASSWORD` and a non-zero bounded `EM_GETPASSWORDCHAR` probe. Do not use a coordinate-only selector.

The credential helper does not bypass Windows UIPI. `SendInput` can target only an equal- or lower-integrity application. If the application is running at a higher integrity level than Desktop MCP, treat the target as incompatible with the ordinary non-elevated Desktop profile; do not silently elevate Desktop or weaken Windows integrity controls as a workaround.
