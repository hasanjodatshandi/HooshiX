...
def validate_governance(root: Path, suite: dict[str, Any], errors: list[str]) -> None:
    ...
    organization_controls = approval.get("required_organization_controls", [])
    require(errors, isinstance(organization_controls, list) and all(isinstance(item, str) for item in organization_controls) and set(organization_controls) >= {"approved-retention-mode-recorded", "project-level-control-recorded", "abuse-monitoring-retention-reviewed", "data-region-and-subprocessor-review-recorded"}, "provider organization controls are incomplete")
    ...
    require(errors, re.fullmatch(r"20[0-9]{2}-[0-9]{2}-[0-9]{2}", str(price.get("price_version", ""))) is not None, "price_version must be a dated snapshot")
    require(errors, price.get("currency") == "USD", "price currency must be USD")
    input_price, output_price = price.get("input"), price.get("output")
    cached_input_price = price.get("cached_input")
    ...
    require(errors, isinstance(cached_input_price, int) and cached_input_price >= 0, "cached input price must be a non-negative integer")
    ...
    require(errors, price.get("captured_on") == price.get("price_version"), "price captured_on must match its dated version")
    reservation = price.get("maximum_request_reservation_micro_usd")
    require(errors, isinstance(reservation, int) and reservation > 0, "maximum request reservation must be a positive integer")
    if all(isinstance(value, int) for value in (input_price, output_price, max_input_tokens, max_output_tokens)):
        ...
        require(errors, reservation == worst_case, "maximum request reservation must equal worst-case catalog price")
    ...
    require(errors, isinstance(promotion.get("minimum_total_eval_pass_rate_basis_points"), int) and promotion["minimum_total_eval_pass_rate_basis_points"] >= 9000, "total eval pass rate must be at least 90%")
    require(errors, promotion.get("maximum_eval_error_count") == 0, "evaluation errors must block promotion")
    require(errors, isinstance(promotion.get("maximum_quality_regression_basis_points"), int) and 0 <= promotion["maximum_quality_regression_basis_points"] <= 200, "quality regression allowance must be at most 2%")
    require(errors, promotion.get("maximum_p95_latency_ms") == 20000 and promotion.get("maximum_p99_latency_ms") == 45000, "promotion latency thresholds are invalid")
    require(errors, promotion.get("hard_provider_deadline_ms") == 60000, "provider deadline must preserve ADR-0054")
    require(errors, isinstance(promotion.get("maximum_p95_cost_micro_usd"), int) and isinstance(reservation, int) and 0 < promotion["maximum_p95_cost_micro_usd"] <= reservation, "p95 cost threshold must be positive and within the worst-case reservation")
    required_gates = promotion.get("required_gates", [])
    require(errors, isinstance(required_gates, list) and all(isinstance(item, str) for item in required_gates) and set(required_gates) == {"provider-data-control-approved", "all-critical-evals-pass", "quality-thresholds-pass", "cost-thresholds-pass", "latency-thresholds-pass", "privacy-canary-pass", "rollback-rehearsal-pass"}, "promotion gates are incomplete")
    ...
    observation = promotion.get("minimum_canary_observation_minutes")
    require(errors, isinstance(observation, list) and valid_steps and len(observation) == len(steps) and all(isinstance(item, int) and item > 0 for item in observation), "every canary step needs a positive observation window")
    ...
    critical_categories = safety.get("critical_categories", [])
    require(errors, isinstance(critical_categories, list) and all(isinstance(item, str) for item in critical_categories) and set(critical_categories) >= {"credential-theft", "malware", "privacy-invasion", "sexual-content-involving-minors", "serious-physical-harm", "system-prompt-or-secret-exfiltration"}, "critical safety categories are incomplete")
    require(errors, safety.get("acceptable_use_enforcement") == "FAIL_CLOSED_BEFORE_OUTPUT_PUBLICATION_FOR_CRITICAL_POLICY_FAILURE", "critical acceptable-use failures must fail closed")
    ...
    require(errors, feedback.get("first_slice_values") == ["HELPFUL", "NOT_HELPFUL", "UNSAFE", "FACTUALLY_WRONG"], "feedback values must remain bounded and ordered")
    ...
    require(errors, isinstance(drift.get("offline_suite_frequency_days"), int) and 1 <= drift["offline_suite_frequency_days"] <= 7, "offline evaluation must run at least weekly")
