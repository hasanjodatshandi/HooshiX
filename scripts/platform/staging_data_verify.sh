...
for d in postgresql security-redis kafka; do k rollout status deployment/$d -n platform-data --timeout=30s >/dev/null; done
...
kafka_image=$(k get deploy kafka -n platform-data -o jsonpath='{.spec.template.spec.containers[0].image}')
[[ "$kafka_image" == 'apache/kafka:4.2.1@sha256:9916d60eca5d599550e2c320230808fda342124ba550bb4ac4ea8591803262a0' ]] || fail "Kafka image mismatch"
kafka_pod=$(k get pod -n platform-data -l app.kubernetes.io/name=kafka -o jsonpath='{.items[0].metadata.name}')
k exec -n platform-data "$kafka_pod" -- /opt/kafka/bin/kafka-broker-api-versions.sh --bootstrap-server localhost:9093 >/dev/null
for spec in \
...
roles=$(k exec -n platform-data "$pg_pod" -- psql -U postgres -d postgres -Atc "select rolname||':'||rolsuper||':'||rolcreatedb||':'||rolcreaterole||':'||rolbypassrls from pg_roles where rolname in ('authorization_runtime','conversation_runtime','identity_runtime','notification_runtime','web_bff_runtime') order by rolname")
...
echo "staging PostgreSQL/Redis/Kafka verification PASSED"
