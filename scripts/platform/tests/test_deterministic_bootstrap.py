...
class DeterministicBootstrapTest(unittest.TestCase):
    ...
    def test_waf_privacy_tests_are_required_by_image_and_ci(self) -> None:
        ...
        self.assertIn("@api path /api /api/*", policy)
        self.assertIn("handle @api", policy)
        self.assertIn("reverse_proxy http://web-frontend.platform-apps.svc.cluster.local:8080", policy)
        self.assertLess(policy.index("handle @api"), policy.index("reverse_proxy http://web-frontend"))
        self.assertIn("header_up -X-HooshiX-Client-IP", policy)
        self.assertNotIn("HOOSHIX_FRONTEND_UPSTREAM", policy)
        self.assertNotIn("HOOSHIX_FRONTEND_UPSTREAM", (ROOT / "infrastructure/waf/kubernetes.yaml").read_text(encoding="utf-8"))
