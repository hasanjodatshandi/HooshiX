...
code=$(curl -sk --resolve localhost:8443:127.0.0.1 -o /dev/null -w '%{http_code}' -X POST https://localhost:8443/api/v1/auth/session/bootstrap -H 'Origin: https://localhost:8443' -H 'Sec-Fetch-Site: same-origin' -H 'Sec-Fetch-Mode: cors' -H 'Sec-Fetch-Dest: empty')
[[ "$code" == 201 ]] || fail "public Traefik -> WAF -> BFF bootstrap expected 201, got $code"
http_code=$(curl -sS --resolve localhost:8080:127.0.0.1 -o /dev/null -w '%{http_code}' http://localhost:8080/ || true)
...
curl -sk --resolve localhost:8443:127.0.0.1 -o /dev/null -X POST https://localhost:8443/api/v1/auth/session/bootstrap -H 'Origin: https://localhost:8443' -H 'Sec-Fetch-Site: same-origin' -H 'Sec-Fetch-Mode: cors' -H 'Sec-Fetch-Dest: empty' -H "Authorization: Bearer $secret_canary" -H "X-HooshiX-Secret-Canary: $secret_canary" || true
code=$(curl -sk --resolve localhost:8443:127.0.0.1 -o /dev/null -w '%{http_code}' \
  "https://localhost:8443/?canary=$secret_canary" \
