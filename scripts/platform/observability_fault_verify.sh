...
trap restore EXIT
smoke(){ local code; code=$(curl -sk --resolve localhost:8443:127.0.0.1 -o /dev/null -w '%{http_code}' -X POST https://localhost:8443/api/v1/auth/session/bootstrap -H 'Origin: https://localhost:8443' -H 'Sec-Fetch-Site: same-origin' -H 'Sec-Fetch-Mode: cors' -H 'Sec-Fetch-Dest: empty'); [[ "$code" == 201 ]] || fail "application failed during telemetry backend outage: HTTP $code"; for d in compromised-password-service notification-service authorization-service identity-service web-bff; do k rollout status deployment/$d -n platform-apps --timeout=20s >/dev/null; done; }
