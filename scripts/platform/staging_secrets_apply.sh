...
for n in authorization-db-migration authorization-db-runtime identity-db-migration identity-db-runtime notification-db-migration notification-db-runtime; do secret_dir platform-data "$n" "$F/$n"; done
for n in authorization-db-migration authorization-db-runtime identity-db-migration identity-db-runtime notification-db-migration notification-db-runtime authorization-quota-redis identity-quota-redis web-bff-redis authorization-fingerprint authorization-quota-key identity-fingerprint identity-challenge identity-handoff identity-mfa identity-quota identity-refresh identity-jwt-private notification-fingerprint notification-delivery web-bff-locator web-bff-csrf web-bff-refresh; do secret_dir platform-apps "$n" "$F/$n"; done
...
for n in authorization identity; do k -n platform-apps create configmap "$n-host-time" --from-literal=host-time-synchronized=synchronized --dry-run=client -o yaml | k apply -f - >/dev/null; done
